...
public class TimerTaskPTI {
	...
	private AccessTableConfigurazioneSessionBeanRemote confService;
	...
	public TimerTaskPTI(String enteId) throws NamingException {
		super();
		this.enteId = enteId;
		operatoreService = (AccessTableOperatoreSessionBeanRemote) ClientUtility.getEjbInterface("CarSocialeA",
				"CarSocialeA_EJB", "AccessTableOperatoreSessionBean");
		...
		confService = (AccessTableConfigurazioneSessionBeanRemote) ClientUtility.getEjbInterface("CarSocialeA",
				"CarSocialeA_EJB", "AccessTableConfigurazioneSessionBean");
	...
	private void processaNuovoInserimento(InserimentoMinoreDaStruttura ins) throws Exception {
		...
		try {
			...
			CsOOperatoreSettore opFittizio = operatoreService.findOperatoreSettore2LivByUsername(opDto);
			if(opFittizio==null){
			...
			CsOSettore settoreStruttura = confService.findSettoreByRelStruttura(dtoS);
			...
			List<CsOOperatoreSettore> opSets = operatoreService.findOperatoreSettoreByCodStruttura(dtoDest);
	...
	private void notificaNuovoDocumento(ArCsPaiPTIDocumentoDTO doc) throws Exception {
		...
		try {
			...
			CsOOperatoreSettore opSet = operatoreService.findOperatoreSettore2LivByIdOperatore(dtoDest);
			...
			csSOSettoreStruttura = confService.findSettoreByRelStruttura(dtoS);
