...
public class ManTestPageBean extends CsUiCompBaseBean {
	...
	public void newCaso() throws Exception {
		try {
			...
			fillEnte(bdto);
			CsACaso caso = casoSessionBean.inizializzaNuovoCaso(bdto);
	...
	protected void loadLastIterStepByCaso() throws Exception  {
		...
		try {
			...
			List<CsACaso> listaCaso = casoSessionBean.findAll(cet);

			for( CsACaso csCaso : listaCaso ) {
				
				BaseDTO bdto = new BaseDTO();
				fillEnte(bdto);
				bdto.setObj(csCaso.getId());
				CsIterStepByCasoDTO itStep = iterSessionBean.getLastIterStepByCasoDTO(bdto);	
				if( itStep != null )
				{
