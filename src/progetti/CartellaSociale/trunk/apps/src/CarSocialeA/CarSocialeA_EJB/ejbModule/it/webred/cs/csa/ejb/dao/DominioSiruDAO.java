...
public class DominioSiruDAO extends CarSocialeBaseDAO implements Serializable {
	...
	public List<SiruDominioDTO> loadAmNazionalita(LuoghiService luoghiService){
		List<SiruDominioDTO> toReturn = new ArrayList<SiruDominioDTO>();
		List<AmTabNazioni> results = luoghiService.getNazioni();
		for(AmTabNazioni obs : results){
			toReturn.add(new SiruDominioDTO(obs.getCodIstatNazione(), obs.getNazionalita()));
		}
		
		return toReturn;
	}
	
	public List<SiruDominioDTO> loadAmNazioneAgEntrate(LuoghiService luoghiService){
		List<SiruDominioDTO> toReturn = new ArrayList<SiruDominioDTO>();
		List<AmTabNazioni> results = luoghiService.getNazioni();
		for(AmTabNazioni obs : results){
			toReturn.add(new SiruDominioDTO(obs.getCodIstatNazione(), obs.getCodAgEntrate()));
		}
