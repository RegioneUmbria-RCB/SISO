...
//				"and (inteseg.id is null or INTESEG.id = (select max(id) from Cs_I_Intervento_Eseg where intervento_id=inter.id)) ";
	...
	private String getSQLCriteria() {
		...
		sqlCriteria += criteria.getDiarioPaiId()!=null ? " AND DIARIO_PAI_ID = "+criteria.getDiarioPaiId() : "";
		sqlCriteria += criteria.isSearchByCaso() && criteria.getCasoId()!=null ? " and CASO_ID = " + criteria.getCasoId() : "";
		sqlCriteria += criteria.getDescCatSociale() != null ? " AND UPPER(DESC_CAT_SOCIALE) LIKE '%" + criteria.getDescCatSociale().toUpperCase() + "%'" : "";
		...
		if(criteria.getLstTipoInterventoInps()!=null){//mk_osmosit
			String lstTipiInps = "";
			for(String val : criteria.getLstTipoInterventoInps())
				lstTipiInps+= "'"+val+"',";
			lstTipiInps = !lstTipiInps.isEmpty()  ? lstTipiInps.substring(0, lstTipiInps.length()-1) : lstTipiInps;
			sqlCriteria += lstTipiInps.equals("") ? "" :  " AND COD_PRESTAZIONE IN ("+lstTipiInps+")";
		}
