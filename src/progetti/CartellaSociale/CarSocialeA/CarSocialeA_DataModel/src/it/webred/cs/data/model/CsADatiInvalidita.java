...
public class CsADatiInvalidita implements Serializable {
	...
	@Id
	@SequenceGenerator(name="CS_A_INVALIDITA_ID_GENERATOR", sequenceName="SQ_ID",allocationSize=1)
	...
	@Column(name="FONTE_DERIVAZIONE_VALUTAZ")
	private Long fonteDerivazioneValutaz;
	...
	//SISO-1278
	//bi-directional many-to-one association to CsAInvCiv
	@OneToMany(mappedBy="csADatiInvalidita", fetch = FetchType.EAGER, cascade = CascadeType.ALL, orphanRemoval=true)
	private List<CsAInvCiv> csAInvCivs;
	
	public CsADatiInvalidita() {
		this.csAInvCivs = new ArrayList<CsAInvCiv>();
	}
	...
	public Long getFonteDerivazioneValutaz() {
		return fonteDerivazioneValutaz;
	}

	public void setFonteDerivazioneValutaz(Long fonteDerivazioneValutaz) {
		this.fonteDerivazioneValutaz = fonteDerivazioneValutaz;
	}

	public List<CsAInvCiv> getCsAInvCivs() {
		return csAInvCivs;
	}

	public void setCsAInvCivs(List<CsAInvCiv> csAInvCivs) {
		this.csAInvCivs = csAInvCivs;
	}

	public CsAInvCiv addCsAInvCiv(CsAInvCiv csAInvCiv) {
		getCsAInvCivs().add(csAInvCiv);
		csAInvCiv.setCsADatiInvalidita(this);

		return csAInvCiv;
	}

	public CsAInvCiv removeCsAInvCiv(CsAInvCiv csAInvCiv) {
		getCsAInvCivs().remove(csAInvCiv);
		csAInvCiv.setCsADatiInvalidita(null);
