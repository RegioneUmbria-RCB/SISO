...
public class ValMultidimensionaleBean extends JsonBaseBean {
	...
	public void setValAutoPers(Integer valAutoPers) {
		this.valAutoPers = valAutoPers;
	}
	...
	public List<String> checkObbligatorieta() {
		...
		if(    !isViveAltri()
			...
			&& !isViveFamiliari()
			&& !isViveFigli()
			&& !isViveSolo() ) messagges.add("Rete familiare: rete familiare è un campo obbligatorio");
		
		if(getValFamRating().intValue() == 0) 
			messagges.add("Rete familiare: Sintesi valutazione rete familiare è un campo obbligatorio");
		
		if(getRelAltriSogg() == null || "".equals(getRelAltriSogg()))
			messagges.add("Rete sociale: Relazioni con altri soggetti è un campo obbligatorio" );
		
		if(getRelAltriSoggRetr() == null || "".equals(getRelAltriSoggRetr()))
			messagges.add("Rete sociale: Relazioni con altri soggetti retribuiti è un campo obbligatorio" );
