...
public class CancellaUtente extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			if (validate(request)) {
				//controllo se è operazione Disable o Ripristina
				...
				if (ripristina)
					...
					//controllo se è stato inserito motivo di Disable
					String motivoDis = request.getParameter("motivoDis") != null?request.getParameter("motivoDis"):"";
					if(!motivoDis.trim().equals("")){
						//richiamo procedura disabled
						BaseAction.CancellaUtente(request);
						
					}else{
						//popup per scrivere motivo disabled
						request.setAttribute("ripristina", request.getParameter("ripristina"));
						request.setAttribute("userName", request.getParameter("userName"));
						request.getRequestDispatcher("/jsp/motivoDisabilita.jsp").forward(request, response);
					}
			...
			for(AmAnagrafica an: listaAnagrafica){
				...
				dto.setStatoPwd("Valida: scadenza " + sdf.format(calUpdPwd.getTime()));
				if (calOggi.getTime().getTime() > calAnteUpdPwd.getTime().getTime())
					dto.setStatoPwd("In scadenza (" + sdf.format(calUpdPwd.getTime()) + ")");
				if (calOggi.getTime().getTime() > calUpdPwd.getTime().getTime())
					dto.setStatoPwd("Scaduta");
