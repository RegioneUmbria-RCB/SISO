    ...
    public javax.ws.rs.core.Response verificaPriK(@QueryParam("priK") String priK) {
		...
		try {
			...
			if(traccia.getUsata() != null && traccia.getUsata().contentEquals("SI")) {
				//Confezionamento del messaggio chiave già usata...
				esitoAut.setEsito(-1001);
				esitoAut.setMessaggio("Attenzione! Accesso non consentito, la chiave è stata già usata. Ripetere la procedura di accesso.");
			}
			else {
				//  messaggio OK 
