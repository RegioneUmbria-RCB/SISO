...
public class RelazioniStampaFiller extends CsUiCompBaseBean {
	...
	private AccessTableMediciSessionBeanRemote mediciService = (AccessTableMediciSessionBeanRemote) getEjb("CarSocialeA", "CarSocialeA_EJB", "AccessTableMediciSessionBean");
	...
	private AccessTableIndirizzoSessionBeanRemote indirizzoS = (AccessTableIndirizzoSessionBeanRemote) getEjb("CarSocialeA", "CarSocialeA_EJB", "AccessTableIndirizzoSessionBean");
	private AccessTableInterventoSessionBeanRemote interventi = (AccessTableInterventoSessionBeanRemote) getEjb("CarSocialeA", "CarSocialeA_EJB", "AccessTableInterventoSessionBean");
	private AccessTableOperatoreSessionBeanRemote operatoreService = (AccessTableOperatoreSessionBeanRemote)getEjb("CarSocialeA", "CarSocialeA_EJB", "AccessTableOperatoreSessionBean");
	private AccessTableDiarioSessionBeanRemote diario = (AccessTableDiarioSessionBeanRemote) getEjb("CarSocialeA", "CarSocialeA_EJB", "AccessTableDiarioSessionBean");
...
public RelazionePdfDTO fillRelazioni() {
		...
		for(CsDIsee d :lstdiario){
				if(d.getCsDDiario().getDtChiusuraDa().after(n))
					 ie = "Isee: "+d.getIsee()+" - " +"Data Scadenza:"+ sdf.format(d.getCsDDiario().getDtChiusuraDa())+ " ";
			is=is+ie;
		}
		...
		if(diarioRelaz.getDiarioId() != null){
			...
			denominazione = soggetto.getCsAAnagrafica().getCognome() + " " + soggetto.getCsAAnagrafica().getNome()+", Data di Nascita"+ soggetto.getCsAAnagrafica().getDataNascita()+ ", Residente in "
					+p.getCsAAnaIndirizzo().getIndirizzo()+", "+ p.getCsAAnaIndirizzo().getCivicoNumero()+" -"+p.getCsAAnaIndirizzo().getComDes()+", Codice Fiscale: "+soggetto.getCsAAnagrafica().getCf();
			...
			try {
				asResponsabile = operatoreService.findOperatoreById(odto);
			} catch (Exception e) {
				logger.error(e);
			}
			...
			if(settDiario!=null){
				...
				try {
					CsOOperatoreSettore cosFirma = operatoreService.findRespSettoreFirma(odto);
					if(cosFirma!=null)
