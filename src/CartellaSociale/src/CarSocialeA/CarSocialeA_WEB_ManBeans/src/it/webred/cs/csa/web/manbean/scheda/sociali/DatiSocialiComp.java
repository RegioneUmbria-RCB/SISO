...
public class DatiSocialiComp extends ValiditaCompBaseBean implements IDatiSociali{
	...
	//private String idGrVulnerabile;
	//private boolean stranieroNonAcc;
    //private BigDecimal codStatus;
	//private BigDecimal codPermesso;
	...
	public List<SelectItem> getLstInviante() {
		...
		if(lstInviante == null){
			...
			lstInviante.add(new SelectItem(null, "- seleziona -"));
			...
			if (lst != null) {
				for (CsOSettore obj : lst) {
					...
					if(obj.getFlgInviante() != null && obj.getFlgInviante() && comuneValido){
...
/*	public List<SelectItem> getLstCaricoA() {
		if(lstCaricoA == null){
			lstCaricoA = new ArrayList<SelectItem>();
			lstCaricoA.add(new SelectItem(null, "- seleziona -"));
			CeTBaseObject bo = new CeTBaseObject();
			fillEnte(bo);
			List<CsOSettore> lst = confService.getSettoreAll(bo);
			LinkedHashMap<String,List<SelectItem>> mappa = new LinkedHashMap<String,List<SelectItem>>();
			...
			if (lst != null) {
				for (CsOSettore obj : lst) {
					String belfioreOrg = obj.getCsOOrganizzazione().getBelfiore();
					boolean comuneValido = belfioreOrg==null || belfioreOrg.equals(bo.getEnteId());
						if(obj.getFlgInCaricoA()!=null && obj.getFlgInCaricoA() && comuneValido){
							String nomeOrg = obj.getCsOOrganizzazione().getNome();
							SelectItem si = new SelectItem(obj.getId(), obj.getNome());
							si.setDisabled(!obj.getAbilitato());
...
/*	public List<SelectItem> getLstInviantoA() {
		if(lstInviantoA == null){
			lstInviantoA = new ArrayList<SelectItem>();
			lstInviantoA.add(new SelectItem(null, "- seleziona -"));
			CeTBaseObject bo = new CeTBaseObject();
			fillEnte(bo);
			List<CsOSettore> lst = confService.getSettoreAll(bo);
			...
			if (lst != null) {
				for (CsOSettore obj : lst) {
					String belfioreOrg = obj.getCsOOrganizzazione().getBelfiore();
					boolean comuneValido = belfioreOrg==null || belfioreOrg.equals(bo.getEnteId());
					if(obj.getFlgInviatoA() != null && obj.getFlgInviatoA() && comuneValido){
						String nomeOrg = obj.getCsOOrganizzazione().getNome();
