	...
	public void init() {
		...
		try {
			...
			if(opSettore!=null){
				...
				//Per ciascuna cat.sociale conteggio il numero di casi registrati/in carico/chiusi dell'organizzazione		
				for(CsRelSettoreCatsoc settCatSoc: listaCatSociali) {
					...
					//Ricerco le categorie sociali del settore selezionato, altrimenti becco tutto anche le altre org.
	...
	public void caricaCaricoLavoro() {
		...
		try {
			...
			for(CsOOperatore op: listaOperatori) {
				...
				Iterator itSett = op.getCsOOperatoreSettores().iterator();
				...
				oComp.setAmAnagrafica(getAnagraficaByUsername(op.getUsername()));
