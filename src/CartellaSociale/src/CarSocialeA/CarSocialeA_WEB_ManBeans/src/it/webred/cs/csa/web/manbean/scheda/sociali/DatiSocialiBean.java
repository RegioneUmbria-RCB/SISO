...
public class DatiSocialiBean extends SchedaValiditaBaseBean implements IDatiValiditaList{
	...
	private void verificaCittadinanza(DatiSocialiComp comp){
	  try{	
		...
		if(anagrafica!=null)
			comp.setStranieriRequired(isVisPanelStranieri() && !"ITALIANA".equals(anagrafica.getCittadinanza()) && 
                                  	  !"ITALIANA".equals(anagrafica.getCittadinanza2()));
	  }catch(Exception e){
	...
	private void popolaDatiSocialiDaSegretariato(DatiSocialiComp comp) throws NamingException{
		...
		if(list.size()>0){
    		...
    		SsSchedaSegnalante segnalante = ssSchedaSegrService.readSegnalanteById(bDto);
    		
    		 if(segnalato!=null){
    			 comp.valorizzaFormazioneLavoroDaUDC(segnalato);
    			 comp.setIdInviante(segnalante.getInviato_da()!=null ? new BigDecimal(segnalante.getInviato_da()) : null);
...
/*		cs.setStranieroNonAcc(comp.getStranieroNonAcc()?"1":"0");
	...
	/*	comp.setIdTipologiaFam(cs.getTipologiaFamiliareId());
		...
		comp.setStranieriRequired(isVisPanelStranieri() && !"ITALIANA".equals(cs.getCsASoggetto().getCsAAnagrafica().getCittadinanza()) && 
                !"ITALIANA".equals(cs.getCsASoggetto().getCsAAnagrafica().getCittadinanza2()));
