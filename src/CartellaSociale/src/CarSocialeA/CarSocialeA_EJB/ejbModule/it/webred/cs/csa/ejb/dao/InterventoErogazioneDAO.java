...
public class InterventoErogazioneDAO extends CarSocialeBaseDAO implements Serializable {
	...
	public CsCfgIntEseg findCfgIntEsegById(Long id) {
       ...
       try{
			...
			if (cfg != null) {
				cfg.getCsCfgAttrUnitaMisuras().size();
				cfg.getCsCfgIntEsegStatos().size();
				cfg.getCsCfgIntEsegAttUms().size();
			}else
	...
	public CsCfgIntEseg findCfgIntEsegByTipoInterventoId(Long tipoInterventoId) {
		...
		try{
			...
			cfg = (CsCfgIntEseg) q.getSingleResult();
			
			if (cfg != null) {
				cfg.getCsCfgAttrUnitaMisuras().size();
				cfg.getCsCfgIntEsegStatos().size();
				for(CsCfgAttrUnitaMisura ums : cfg.getCsCfgAttrUnitaMisuras())
					ums.getCsCfgAttributo().getCsCfgAttrOptions().size();
			}else
	...
	public List<CsCfgIntEsegStato> getListaIntEsegStatoByTipiStato(List<String> tipiStato, Long tipoInterventoId) {
		Query q = em.createNamedQuery("CsCfgIntEsegStato.findIntEsegStatoByTipiStato");
	...
	@SuppressWarnings("unchecked")
	public List<CsIInterventoEseg> getInterventoEsegByIntervento(Long interventoId) {
		List<CsIInterventoEseg>listaInterventi;
		try{
			Query q = em.createNamedQuery("CsIInterventoEseg.findInterventoEsegByIntervento");
	...
	@SuppressWarnings("unchecked")
	public List<CsIInterventoEseg> getInterventoEsegByIdMaster(Long masterId) {
		List<CsIInterventoEseg>listaInterventi;
		try{
			...
			throw new CarSocialeServiceException("getInterventoEsegByMasterId",e);
		}
		return listaInterventi;
	}
	...
	public void aggiungiInterventoEseguito(List<CsIInterventoEseg> intEsegNuovo) {
		for (CsIInterventoEseg csIInterventoEseg : intEsegNuovo) {			
			em.persist(csIInterventoEseg);
		}

	}
	public void aggiungiInterventoEseguitoMast(CsIInterventoEsegMast intEsegMastNuovo) {		
			em.merge(intEsegMastNuovo);		
	}
...
/*	public List<DatiAggregatiErogazioneDTO> searchListaErogInterventiBySettoreOLD(ErogazioniSearchCriteria bDto) throws CarSocialeServiceException {
	...
	public List<CsIInterventoEsegMastSogg> findMastSoggSenzaCaso(){
		List<CsIInterventoEsegMastSogg> lst = new ArrayList<CsIInterventoEsegMastSogg>();
		try{
			Query q = em.createNamedQuery("CsIInterventoEsegMastSogg.findSenzaCaso");
			 lst = q.getResultList();
		}catch(Exception e){
			logger.error(e.getMessage(),e);
			throw new CarSocialeServiceException(e);
