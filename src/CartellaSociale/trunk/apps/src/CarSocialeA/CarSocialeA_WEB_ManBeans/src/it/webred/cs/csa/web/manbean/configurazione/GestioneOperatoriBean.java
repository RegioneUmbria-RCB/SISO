...
public class GestioneOperatoriBean extends CsUiCompBaseBean implements IGestioneOperatori, Serializable {
	...
	private void caricaLstoperatori() {
		...
		try {
			...
			for(CsOOperatore op: listaOp) {
				AmAnagrafica amAna = getAnagraficaByUsername(op.getUsername());
				if(amAna != null) {
	...
	public void ricercaOperatore() {
		try {
			...
			AmAnagrafica ana = lst.get(0);
			if(!ana.getCodiceFiscale().equals(ana.getAmUser().getName())){
				addWarning("Codice fiscale ("+ana.getCodiceFiscale()+") e identificativo utente ("+ana.getAmUser().getName()+") non corrispondono", "");
				return;
			}
	...
	public void salvaOperatore() {
		try {
			...
			input.setObj(op);
			input.setObj2(update);
			operatoreService.insertOrUpdateOperatore(input);
	...
	public void aggiungiOperatoreSettore() {
		try {
			...
			CsOOperatoreSettore opSet = getCsOOperatoreSettore(insUpdDelGestOperatoriBean.getSelSettore());
			...
			input.setObj(opSet);
			input.setObj2(update);
			opSet = operatoreService.insertOrUpdateOperatoreSettore(input);
			...
			if (update && aggiornaTipoOp) {
				...
				if (tipoOp != null && aggiornaTipoOp) {
					...
					input.setObj(tipoOp);
					input.setObj2(true);
					operatoreService.insertOrUpdateTipoOperatore(input);
				}
			}
			if (aggiornaTipoOp) {
		...
		try {
			...
			for (DataTableCsOOperatoreSettore selOpSet : selOpSets) {
				...
				if (opSet != null) {
					...
					input.setObj(opSet);
					input.setObj2(true);
					operatoreService.insertOrUpdateOperatoreSettore(input);
				}
			}
			getInsUpdDelGestOperatoriBean().loadDataTable(getRicercaGestOperatoriBean().getOperatore().getIdOperatore());
