	...
	public void init() {
		...
		try {
			...
			visualCaricoLavoro = checkPermesso(PermessiErogazioneInterventi.ITEM,
					DataModelCostanti.PermessiCaso.VISUALIZZAZIONE_CARICO_LAVORO);
	...
	public void caricaCaricoLavoro() {
		try {
			...
			for (CsOOperatore op : listaOperatori) {
				...
				dto.setObj(op.getId());
				// SISO-640
				Integer numeroCasi = casoService.countCasiByResponsabileCatSociale(dto);
				if (numeroCasi > 0 && operatorCanBeAdded) {
					oComp.setNumCasi(numeroCasi);
					lstCasiOperatore.add(oComp);
				}else {
					logger.info(String.format("operatore %d <%s> saltato", oComp.getOperatore().getId(), oComp.getOperatore().getDenominazione()));
				}
			// -----------------------------------------------------------------------~ SISO-640
			}
