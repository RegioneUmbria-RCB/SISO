	...
	public void carica(CsASoggettoLAZY soggetto) {
		...
		if(soggetto != null) {
				...
				*/
				...
				if(checkPermesso(DataModelCostanti.PermessiCaso.ITEM, DataModelCostanti.PermessiCaso.MODIFICA_CASI_SETTORE) 
				...
				if(schedaSegr != null){
					...
					CsSsSchedaSegr ss = ssService.findSchedaSegrById(dto);
					catSocSegr = ss.getCsCCategoriaSociale().getDescrizione();
					itemSelezionato = ss.getCsCCategoriaSociale().getId()+"|"+ss.getCsCCategoriaSociale().getDescrizione();
				}
		...
		try {
			...
			if (beanLstCatSociali != null) {
				for (CsRelSettoreCatsoc cat : beanLstCatSociali) {
				    ...
				    if(cat.getAbilitato()!=null && cat.getAbilitato().intValue()==1)
				    	lstItems.add(si);
				}
			}
		} catch (NamingException e) {
