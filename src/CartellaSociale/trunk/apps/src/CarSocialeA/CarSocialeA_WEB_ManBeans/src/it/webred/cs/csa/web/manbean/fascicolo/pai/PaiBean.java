...
public class PaiBean extends FascicoloCompBaseBean implements IPai, Serializable {
	...
	public void onChangeTipoProgetto(AjaxBehaviorEvent event){
		...
		long tipoProgettoId = Long.valueOf((String)som.getSubmittedValue());
	...
	public void initializeData() {
		try {
			...
			//SISO-155
			tipoProgetto = null;
			progettiPai = diarioService.findListaProgettiPai();
	...
	public void carica() {
		...
		try {			
			...
			if(tipoProgetto != null && tipoProgetto.equals(PaiProgettiEnum.AFFIDO.getProgetto())){
				paiAffidoBean.findAffidoByPai(selectedPai.getDiarioId());
			}
...
//		}
	...
	public List<ErogInterventoRowBean> getLstErogazioni(){
		...
		bDto.setSearchErogatiNoIntervento(true);
...
//			RequestContext.getCurrentInstance().update("@([id$=paiTabViewContainer])");
	...
	public void refreshPicklistErogazioni(Long idMast){
    	...
    	String id="E"+idMast;
    	if (picklistErogazioni!=null) {		//è null quando il salvataggio viene richiamato dalla tab "Interventi"; è valorizzato quando viene richiamato nella tab PAI
    		Iterator it = this.picklistErogazioni.getSource().iterator();
    		while(it.hasNext()){
    			ErogInterventoRowBean row = (ErogInterventoRowBean)it.next();
    			if(row.getIdRow().equals(id)){
