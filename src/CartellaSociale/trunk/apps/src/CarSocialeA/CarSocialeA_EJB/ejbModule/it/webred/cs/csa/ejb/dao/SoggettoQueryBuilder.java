...
/*		if(!count)
	...
	private String getSQLCriteria() {
		...
		if(criteria.getTribunale()!=null){
			sqlCriteria += Arrays.asList(criteria.getTribunale()).contains("TMCivile") ? " AND TR.TM_CIVILE=1 " : "";
			sqlCriteria += Arrays.asList(criteria.getTribunale()).contains("TMAmministrativo") ? " AND TR.TM_AMM=1 " : "";
			sqlCriteria += Arrays.asList(criteria.getTribunale()).contains("penaleMinorile") ? " AND TR.PENALE_MIN=1 " : "";
			sqlCriteria += Arrays.asList(criteria.getTribunale()).contains("NIS") ? " AND TR.NIS=1 " : "";
			sqlCriteria += Arrays.asList(criteria.getTribunale()).contains("procuraMinorile") ? " AND TR.PROCURA_MIN=1 " : "";
			sqlCriteria += Arrays.asList(criteria.getTribunale()).contains("TO") ? " AND TR.TO_FLG=1 " : "";	
		}
