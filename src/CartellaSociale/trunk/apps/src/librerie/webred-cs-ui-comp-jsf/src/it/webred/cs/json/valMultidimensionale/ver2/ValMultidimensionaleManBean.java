...
//		sinaService.
	...
	public void radioSelect(){
	    ...
	    if(risp.containsKey(Long.valueOf(7)) && risp.containsValue("20")){
	    	this.getJsonCurrent().setAssenzaReteFam(true);
	    	
	    }
	    else if(risp.containsKey(Long.valueOf(7)) && !risp.containsValue("20")){
	    	this.getJsonCurrent().setAssenzaReteFam(false);
	    }
		...
		*/
		...
		try {
			if (validaData()) {
				...
				//controller.getDiarioId();
				
				CsUiCompBaseBean.fillEnte(sinaDTO);
				sinaDTO.associaDiario(controller.getDiarioId());
	...
	public ReportPdfDTO fillReport(String reportPath, List<String> listaSubreport, HashMap<String, Object> map, List<IIseeJson> lstIsee, ISchedaBarthel barthelMan) {
			...
			for(CsDSinaEseg s :sinaDTO.getCsDSina().getCsDSinaEseg()){
			...
			for(ArTbPrestazioniInps ar : sinaDTO.getCsDSina().getArTbPrestazioniInps()){
			...
			if(this.getJsonCurrent().reteFamNonCollaborativa)
				lstViveCon.add(new ValoriPdfDTO("Rete Familiare non Collaborativa","[Rete familiare con elevata conflittualità e/o multi problematicità, non collaborativa che richiede un impegnativo coinvolgimento e la presa in carico di uno o pi� servizi]",this.getJsonCurrent().getDaQuandoSituazione()));
	...
	public void setLstSinaParamInvalidita(
			List<CsTbSinaDomanda> lstSinaParamInvalidita) {
