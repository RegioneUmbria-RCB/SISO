...
public class InterventoErogazHistoryBean extends CsUiCompBaseBean implements Serializable {
	...
	private void builHistoryRows() {
		...
		for (CsIInterventoEseg csIInterventoEseg : csIInterventoEsegLst) {
			InterventoErogazHistoryRowBean curHistoryRow = new InterventoErogazHistoryRowBean(header, csIInterventoEseg, mappa);
	...
	public void addNew(CsOSettore sett, String descrizioneErogazione, CsCfgIntEsegStato stato, CsIInterventoEseg nuovoCsIntervento, HashMap<Long,List<InterventoErogazAttrBean>> mappa) {
		if (canAdd(stato)) {
			//TODO
