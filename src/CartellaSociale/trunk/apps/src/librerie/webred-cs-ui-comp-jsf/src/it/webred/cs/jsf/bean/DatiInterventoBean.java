...
public class DatiInterventoBean extends CsUiCompBaseBean implements Serializable {	
	...
	//SISO-745
	List<SelectItem> listaParenti = null;
	
	private void initCsISemiResiMin(){
		srm = new CsISemiResiMin();
		srmSpeseExtra = new ArrayList<String>();
		//SISO-745
		srmComponente = new ComponenteAltroMan(idSoggetto,listaParenti);
	...
	private void initCsIBuonoSoc(){
		...
		//SISO-745
		buonoSocComponente = new ComponenteAltroMan(idSoggetto,listaParenti);
	}
	
	private void initCsIContrEco(){
		contrEco = new CsIContrEco();
		//SISO-745
		contrEcoComponente = new ComponenteAltroMan(idSoggetto,listaParenti);
	}
	...
	private void initCsIResiMinore(){
		...
		//SISO-745
		rmComponente = new ComponenteAltroMan(idSoggetto,listaParenti);
	}
	
	private void initCsIAdmAdh(){
		adm = new CsIAdmAdh();
		//SISO-745
		admComponente = new ComponenteAltroMan(idSoggetto,listaParenti);
	}
	...
	private void initDettagliIntervento(){
		...
		//SISO-745
		listaParenti = new ArrayList<SelectItem>(buonoSocComponente.getLstParenti());
	...
	public DatiInterventoBean(Long settoreId, Long idSoggetto){
		...
		//INIZIO SISO-391
		this.setTipoDataInizio(DataModelCostanti.DatiInterventoConst.TIPO_DATA_CERTA);
		this.setTipoDataFine(DataModelCostanti.DatiInterventoConst.TIPO_DATA_CERTA);
		//FINE SISO-391
	...
	public DatiInterventoBean(CsIIntervento ci, Long idSoggetto){
		...
		//INIZIO SISO-391 
		if (ci.getInizioDal()!=null && ci.getInizioAl()!=null) { 
			this.setTipoDataInizio(DataModelCostanti.DatiInterventoConst.TIPO_DATA_PRESUNTA);
		} else{
			this.setTipoDataInizio(DataModelCostanti.DatiInterventoConst.TIPO_DATA_CERTA);
		}

		if (ci.getFineDal()!=null && ci.getFineAl()!=null) { 
			this.setTipoDataFine(DataModelCostanti.DatiInterventoConst.TIPO_DATA_PRESUNTA);
		} else { 
			this.setTipoDataFine(DataModelCostanti.DatiInterventoConst.TIPO_DATA_CERTA);
		} 
		//FINE SISO-391
		...
		if(this.getComponente()==null){
			//SISO-745
			this.setComponente(new ComponenteAltroMan(idSoggetto,listaParenti));
		}
...
//		}
	...
	//INIZIO SISO-391
	public Boolean getDettaglioInterventoModifica() {
		return true;	//TODO si deciderà in futuro quando inibire la modifica dei dati di dettaglio
	}
	//FINE SISO-391
	...
	public void setContrEco(CsIContrEco contrEco) {
		...
		if(contrEco!=null){
			...
			if(contrEco.getCsAComponente()!=null)
	...
	public void setBuonosoc(CsIBuonoSoc buonosoc) {
		...
		if(buonosoc!=null){
			...
			if(buonosoc.getCsAComponente()!=null)
	...
	public void setSl(CsISchedaLavoro schedal) {
		...
		try{
			AccessTableConfigurazioneSessionBeanRemote confService = (AccessTableConfigurazioneSessionBeanRemote) getCarSocialeEjb ("AccessTableConfigurazioneSessionBean");	
			BaseDTO dto = new BaseDTO();
			fillEnte(dto);
	...
	private void valorizzaResiMinore() throws NamingException{
		...
		CsTbTipoRetta tipo = null;
		if(this.rmTipoRetta!=null){
			b.setObj(this.rmTipoRetta);
			tipo  = confService.getTipoRetta(b);
		}
		this.minore.setCsTbTipoRetta(tipo);
	...
	private void valorizzaSemiResiMinore() throws NamingException{
		...
		CsTbTipoRetta tipo = null;
		if(this.srmTipoRetta!=null){
			b.setObj(this.srmTipoRetta);
			tipo  = confService.getTipoRetta(b);
		}
		this.srm.setCsTbTipoRetta(tipo);
	...
	private void valorizzaResiAdulti() throws NamingException{
		...
		CsTbTipoRetta tipo = null;
		if(this.raTipoRetta!=null){
			b.setObj(this.raTipoRetta);
			tipo  = confService.getTipoRetta(b);
		}
		this.resiAdu.setCsTbTipoRetta(tipo);
...
//	}
	...
	public void inizializzaNuovoTipoIntervento(Long idTipoIntervento, Long idTipoIntrCustom, Long idCatSociale) throws Exception {
		this.idTipoIntervento = idTipoIntervento;
		this.idTipoIntrCustom = idTipoIntrCustom;
		this.idCatSociale = idCatSociale;
		...
		if( idTipoIntervento != null ){
			...
			CsCTipoIntervento ti = interventoService.getTipoInterventoById(dto);
	...
	public void onChangeTipoRiscossione(){
		...
		}else if(!"Accredito".equalsIgnoreCase(contrEco.getTipoRiscossione())){
			contrEco.setIban(null);
			contrEco.setAccreditoA(null);
		}
	}
