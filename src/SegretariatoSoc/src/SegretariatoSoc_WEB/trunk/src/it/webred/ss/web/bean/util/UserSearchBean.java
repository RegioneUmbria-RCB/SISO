	...
	public void nuovaDaAnagrafe(String id) {
		...
		try {
			...
			if(p != null) {
				...
				//stato civile
				CsTbStatoCivile csTbStatoCivile = this.nuovaSchedaWizard.getStatoCivileByIdOrigCeT(p.getStatoCivile());
...
/*	@Override
	...
	public void setIdSoggetto(String idSoggetto) {
		 l'autocomplete che usa questo bean si trova sul wizard di creazione/modifica scheda UDC
		 * se mettiamo un valore in idSoggetto, quando nel wizard ricarichiamo la pagina
		 * in cui è contenuto l'autocomplete, si ha un errore dato che JSF tenta di convertire idSoggetto
		 * in un oggetto var usato dal componente autocomplete.
		 * All'interno dell'autocompolete var è usato nella seguente maniera:
		 * var="userVar" itemLabel="#{userVar.itemLabel}" itemValue="#{userVar.id}"
		 * userVar conterrà idSoggetto che è una stringa, ma le stringhe non hanno la proprietà itemLabel, pertanto si avrà un errore
