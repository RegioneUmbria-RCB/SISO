...
public class SearchBean extends SegretariatoSocBaseBean{ 
	...
	public void searchSoggetto(ActionEvent actionEvent){
		...
		if((cognome == null && nome == null && cf == null ) || (cognome.isEmpty() && nome.isEmpty() && cf.isEmpty()))
			...
			for(String k: keys){
				...
				valorizzaPresenzaInCS(s.getCf(), s);
	...
	private void verificaPrivacy(Soggetto val){
		try {
			...
			dto.setObj(val.getCf());
			String org = this.getPreselectedOrganizzazione();
			dto.setObj2(new Long(org.split("\\|")[1]));
			SsSchedaPrivacy sp = getSsSchedaService().findSchedaPrivacyByCfEnte(dto);
			String privacy = (sp!=null && sp.getDtIns()!=null) ? this.getMessaggioPrivacy(sp.getDtIns()) : null;
			val.setPrivacy(privacy);
		} catch (Exception e) {
...
///fine implementazione
	...
	public void valorizzaPresenzaInCS(String cf, Soggetto s){
    	...
    	try {
    		...
    		if(settore!=null){
    			...
    			String nomeOrg = o!=null && o.getBelfiore()!=null ? "Comune di "+o.getNome() : o.getNome();
