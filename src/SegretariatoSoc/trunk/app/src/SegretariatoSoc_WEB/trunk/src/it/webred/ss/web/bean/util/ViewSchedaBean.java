...
public class ViewSchedaBean extends SegretariatoSocBaseBean {
	...
	public ViewSchedaBean(){
		...
		try {
			...
			SsTipoScheda tipoBean = schedaService.readTipoSchedaById(dto);
			...
			if(segnalante != null){
				...
				CsOSettore settEnte = configService.getSettoreById(dtoCS);
				...
				if(Scheda.Accessi.Interlocutori.UTENTE.equalsIgnoreCase(a.getInterlocutore())) 
					hideSegnalante=true;	
			...
			serviziRichiestiInterventiCustomBean = new ServiziRichiestiInterventiCustomBean(null, s);  //SISO-438 
			serviziRichiestiInterventiCustomBean.loadManJsonServiziRichiesti(s  //SISO-438 
							//, false
						);  
        	...
        	for(SsAnagrafica ana: anagrafiche){
            	...
            	for(SsDiario nota: diario)
            		note.add(new Nota(nota, this.getCognomeNomeUtente(nota.getAutore()), 
            				canReadNotaDiario(nota, a.getOperatore()), false));
