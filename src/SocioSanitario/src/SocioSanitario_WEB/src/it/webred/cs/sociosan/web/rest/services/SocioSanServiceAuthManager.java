...
public class SocioSanServiceAuthManager implements IAuthManager {
	...
	public boolean isAuthorizated(BaseRequest req) {
		...
		if (authreq.getSessionId() !=null) {
			ValidationDBReader valAuth = new ValidationDBReader();
			boolean isTokenValid = false;
			try {
				isTokenValid = valAuth.checkLogin(authreq);
			} catch (Exception e) {
				logger.error("Problemi nel controllo autenticazione per sessionid = " + authreq.getSessionId(),e);
				return false;
			}
			if (isTokenValid)
				return true;
