...
import it.webred.ss.ejb.SsSchedaSessionBean;
...
public class CTConfigClientSessionBeanTest {
	...
	public void testSendSimpleMailFromSISO() {
		...
		try {
			sendsisomail = ClientUtility.getEJBInterfaceForRemoteClient(ArgoBufferManagerSessionBeanRemote.class , "SocioSanitario", "SocioSanitario_EJB", "ArgoBufferManagerSessionBean", "");
	...
	public void testSISOMailParameters() {
		...
		try {
			confTest = ClientUtility.getEJBInterfaceForRemoteClient(CTConfigClientSessionBeanRemote.class , "SocioSanitario", "SocioSanitario_EJB", "CTConfigClientSessionBean", "");
		} catch (NamingException e1) {
		...
		try {
			mailParams = confTest.getSISOMailPatametres();
	...
	public void testCallRemoteEJB() {
		...
		try {
			ssSchedaSegrService = ClientUtility.getEJBInterfaceForRemoteClient( SsSchedaSessionBeanRemote.class , "SegretariatoSoc", "SegretariatoSoc_EJB", "SsSchedaSessionBean", "", "localhost");
		} catch (NamingException e1) {
