...
public class NuovaSchedaWizard extends SegretariatoSocBaseBean {
	...
	public NuovaSchedaWizard() {
		...
		this.capofila = configurationCsEnteBean.getOrganizzazioneCapofila(cet);
...
//	    	segnalato.getAnagrafica().getComuneNazioneNascitaMan().setNazioneValue();
	...
	private void loadTipiScheda() {
		try {
			...
			BaseDTO dto = new BaseDTO();
			fillUserData(dto);
			this.tipiSchedaModel = confService.readTipiScheda(dto);
...
//				tipiScheda.add(invioGroup);
	...
	public List<SelectItem> getInviatiDa() {
		if (inviatiDa.isEmpty()) {
			try {
				...
				CeTBaseObject bo = new CeTBaseObject();
				fillUserData(bo);
				List<CsOSettore> lst = configurationCsEnteBean.getSettoreAll(bo);
	...
	public void salvaSchedaCompletaUDC() {
		Long idScheda = scheda != null ? scheda.getId() : null;
		boolean validaPic = validaPresaInCarico();
		boolean validaPor = validaDatiPor();
		if (validaPic  && validaPor) {
			...
			logger.warn("salvaSchedaCompletaUDC - validazione pic["+validaPic+"] por["+validaPor+"]");
		}
	...
	private boolean saveAccessoIntoDB(boolean validate) {
		...
		try {
			...
			dto.setObj(relPK);
			SsRelUffPcontOrg rel = confService.getSsRelUffPcontOrg(dto);
...
/*	private void initFromAnonimo() throws Exception {
	...
	private boolean readUfficiFromDB(List<SelectItem> uffici) {
		try {
			...
			List<SsUfficio> results = confService.readUffici(dto);
	...
	public List<SelectItem> getCittadinanzeAcquisite() {
		if (cittadinanzeAcquisite.isEmpty()) {
			try {
				CeTBaseObject cet = new CeTBaseObject();
				this.fillUserData(cet);
				List<it.webred.cs.csa.ejb.dto.KeyValueDTO> lst = configurationCsBean.getCittadinanzeAcquisite(cet);
				cittadinanzeAcquisite = CsUiCompBaseBean.convertiLista(lst);
...
//		this.organizzazioniDiZona = new ArrayList<SelectItem>();
		...
		if (organizzazioniDiZona == null || organizzazioniDiZona.isEmpty()) {
			try {
				BaseDTO dto = new BaseDTO();
				fillUserData(dto);
				List<SsOOrganizzazione> orgs = confService.readOrganizzazioniZona(dto);
				this.organizzazioniDiZona = new ArrayList<SelectItem>();
	...
	public List<SelectItem> getOrganizzazioniAltre() {
		if (organizzazioniAltre == null || organizzazioniAltre.isEmpty()) {
			...
			try {
				BaseDTO dto = new BaseDTO();
				fillUserData(dto);
				List<SsOOrganizzazione> orgs = confService.readOrganizzazioniAltre(dto);
	...
	private SsOOrganizzazione readSsOrganizzazione(long organizzazioneDiZonaId2) {
		try {
			...
			return confService.readSsOOrganizzazioniById(dto);
	...
	private OrganizzazioneDTO readSsArOrganizzazione(long organizzazioneId, boolean orgZonaSociale) {
		...
		try {
			...
			return confService.findOrganizzazioneDestInvio(dto);
		...
		if (ufficiOrganizzazione == null || ufficiOrganizzazione.isEmpty() || reloadUfficiOrganizzazione) {
			...
			if (organizzazioneDiZonaId >= 0) {
				try {
					...
					List<SsRelUffPcontOrg> ufficiOrgRel = confService.readUfficiOrganizzazione(dto);
		...
		if (ufficioDest == null || ufficioDest.getId() != ufficioOrganizzazioneId) {
			if (ufficioOrganizzazioneId > -1) {
				try {
					...
					this.ufficioDest = confService.readUfficio(dto);
...
//		}
   ...
   public void esciAction(){
	...
	protected void loadTipoInterventi() {
		...
		fillEnte(dto);
		this.lstTipoInterventiCustom = getConfigurationCsBean().findTipiIntCustom(dto);
	...
	protected CsCTipoInterventoCustom findTipoInterventoCustom(Long id){
		...
		fillEnte(dto);
		dto.setObj(id);
		return getConfigurationCsBean().findTipoInterventoCustomById(dto);
	}
...
//		this.privacyAnonimo = privacyAnonimo;
	...
	public void caricaListaSettoriByIntervento(Long idTipoIntervento) {
		try {
			...
			if (idTipoIntervento != null) {
				...
				List<it.webred.cs.csa.ejb.dto.KeyValueDTO> listaSettori = configurationCsEnteBean.findSettoriByInterventoCustom(dto);

				//lista dei settori per riempire tendina
				listaSettoriEroganti = CsUiCompBaseBean.convertiLista(listaSettori);
	...
	public void onSettEroganteChange(AjaxBehaviorEvent event){
		...
		if (!StringUtils.isBlank(this.selSettore.getCodice())){
			
			it.webred.cs.csa.ejb.dto.BaseDTO dto = new it.webred.cs.csa.ejb.dto.BaseDTO();
			this.fillUserData(dto);
			dto.setObj(Long.parseLong(this.selSettore.getCodice()));
			selectedSettore = configurationCsEnteBean.getSettoreById(dto);
			
			if (selectedSettore != null && listaSettoriEroganti != null && listaSettoriEroganti.size() > 0) {
	...
	public void inizializzaEStampaModelloPOR() {
		...
		if(this.segnalato.getResidenza()!=null){
			ComuneNazioneGenericMan man = this.segnalato.getResidenza().getComuneNazioneMan();
			if(man.isComune()){
				...
				datiProgettoBean.setComuneResidenza(man.getNazioneMan().getNazione().getNazione());
			}
			this.datiProgettoBean.setViaResidenza(this.segnalato.getResidenza().getVia());
		}
		
		if(this.segnalato.getDomicilio()!=null) {
			ComuneNazioneGenericMan man = this.segnalato.getDomicilio().getComuneNazioneMan();
			if(man.isComune()){
				...
				datiProgettoBean.setDomicilioComune(man.getNazioneMan().getNazione().getNazione());
			}
		...
		this.datiProgettoBean.setTitoloStudioTooltip(this.segnalato.getFormLavoroMan().getTitoloStudioTooltip());
