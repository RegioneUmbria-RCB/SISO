...
public class Segnalante implements IComune, CommonDatiAnaBean {
	...
	public SsSchedaSegnalante fillModel(){
		...
		if(!this.isEmpty()){
			...
			model.setAffidatario(affidatario); 
		}
		return model;
	}
	...
	public void initFromModel(SsSchedaSegnalante segnalante, boolean copia) {
		if(segnalante != null){
			...
			affidatario = segnalante.getAffidatario();  //SISO-906
		}
	}
	...
	public void initFromAnaEsterna(PersonaDettaglio p, CsTbStatoCivile statoCivile) throws Exception {
		...
		if(p != null) {
			...
			//indirizzo res	
			indirizzo = p.getIndirizzoCivicoResidenza();
			comune = new ComuneBean(p.getComuneResidenza());

			//stato civile
			codStatoCivile = statoCivile!=null ? statoCivile.getCod() : null;
			
		}
	}
	
	public String getStampaDesComuneResidenza(){
		return this.comune != null ? 
