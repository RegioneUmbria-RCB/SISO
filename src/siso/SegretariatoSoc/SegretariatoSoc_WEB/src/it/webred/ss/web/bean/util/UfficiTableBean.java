...
/*		PuntoContatto pcontatto  = getPreselectedPContatto();
    ...
    private boolean populateUffici(Long id) {  
    	...
    	try {
        	...
        	List<SsRelUffPcontOrg> results = confService.readUffPcontByOrganizzazione(dto);
        	...
        	for(SsRelUffPcontOrg rel: results){
        		...
        		if(!mappaUffici.containsKey(ufficio.getId().toString())){
	        		...
	        		int ricevute  = countSchedeRicevute(id, ufficio.getId()); 
	        		
	        		Ufficio uffDto = new Ufficio(ufficio, totale, sospese, inCarico, eliminate, new Long(ricevute));
    ...
    public void vaiASchedePerUfficio(Ufficio ufficio){
    	...
    	if(getPreselectedPContatto()==null || this.getPreselectedPContatto().getIdPContatto()==null || this.getPreselectedPContatto().getIdPContatto()<=0)	
	        ...
	        this.selectedUfficio=ufficio;
			String url = addParameter(SCHEDE_INVIATE_URL, "ufficio", selectedUfficio.getId()+"");
			try {
				FacesContext.getCurrentInstance().getExternalContext().redirect(url);
			} catch (IOException e) {
				logger.error(e);
	...
	private int countSchedeRicevute(Long organizzazioneId, Long ufficioId) {
		int countSchede = 0;
		try{
			SsSearchCriteria dto = new SsSearchCriteria();
			fillUserData(dto);
			dto.setUfficioId(ufficioId);
			dto.setZonaSociale(UserBean.getBeanIstance().getZonaSociale());
			dto.setOrganizzazioneId(organizzazioneId);
			countSchede = this.getSsSchedaService().countSchedeInviateUfficioNonLette(dto);
