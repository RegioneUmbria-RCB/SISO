...
public class Accesso {
	...
	public void fillModel(SsSchedaAccesso accessoModel) {
		accessoModel.setId(this.id);
		accessoModel.setData(this.dataAccesso);
		accessoModel.setOperatore(this.operatore);
		accessoModel.setModalita(this.modalitaAccesso);
		accessoModel.setInterlocutore(this.interlocutore);
		accessoModel.setDescrizione(this.descrizione);
		...
		accessoModel.setInviatoDa(this.inviante != null && this.inviante > 0 ? this.inviante : null);
		if (accessoModel.getUtenteAccompagnato() != null && accessoModel.getUtenteAccompagnato() == true) {
			accessoModel.setAccompagnatore(this.accompagnatore);
		}
		//FINE SISO-346
	}
	...
	public void initFromModel(SsSchedaAccesso accesso) {
		if (accesso != null) {
			this.id = accesso.getId();
			this.dataAccesso = accesso.getData();
			this.operatore = accesso.getOperatore();
			this.descrizione = accesso.getDescrizione();
			this.modalitaAccesso = accesso.getModalita();
			this.interlocutore = accesso.getInterlocutore();
			this.motivo = accesso.getMotivo();
			this.motivoDesc = accesso.getMotivoDesc();
			this.puntoContatto.initFromModel(accesso.getSsRelUffPcontOrg());
			this.utenteAccompagnato = accesso.getUtenteAccompagnato();
			this.utentePresenteInformato = accesso.getUtentePresenteInformato();
			this.inviante = accesso.getInviatoDa(); //SISO-346
			this.accompagnatore = accesso.getAccompagnatore(); //SISO-346
		}
	...
	public void initFromModelAccessoInviante(SsSchedaAccessoInviante accesso) {
		if (accesso != null) {
			this.id = accesso.getId();
			this.dataAccesso = accesso.getOrigDataAccesso();
			this.operatore = accesso.getOrigCognomeOperatore() + " " + accesso.getOrigNomeOperatore();
			this.descrizione = "---";
			this.modalitaAccesso = accesso.getOrigAccessoModalita();
			this.interlocutore = accesso.getOrigAccessoIneterlocutore();
			this.motivo = accesso.getOrigAccessoMotivo();
			this.motivoDesc = "---";
			this.puntoContatto = new PuntoContatto();
			...
			this.puntoContatto.setOrganizzazione(organizzazione);

			Ufficio ufficio = new Ufficio();
			ufficio.setNome(accesso.getOrigNomeUfficio());
			ufficio.setId(accesso.getOrigIdUfficio());
			this.puntoContatto.setUfficio(ufficio);
			this.puntoContatto.setNomePContatto(accesso.getOrigPuntoContatto());
		}
