...
//			Leggo i settori per Riunione
	...
	private void loadCatalogoMacroAttivita(){
		...
		fillEnte(cet);
		for (CsTbMacroAttivita macro : confService.getMacroAttivita(cet)) {
	...
	private void loadCatalogoProblematiche(){
		...
		for (CsTbProbl problematica : confService.getProbl(cet)) {
	...
	//CARICA La Lista Settori come partecipanti alla Riunione
		...
		if(listaSettoriRiunione == null ) {
			...
			listaSettoriRiunione = confEnteService.getSettoreRiunione(bo);
		}
	...
	protected void loadTipoInterventi() {
		...
		BaseDTO dto = new BaseDTO();
		fillEnte(dto);
		List<KeyValueDTO> lstTipoInterventi = confService.findAllTipiIntervento(dto);
...
//		listaTipiInterventoAttivi = new ArrayList<CsCTipoIntervento>();
	...
	private void loadListaRelazioniDTO(){
		listaRelazioniDTO = new ArrayList<RelazioneDTO>();
		...
		if (fromFascicoloCartellaUtente) {
			...
			dto.setObj(cf);
			anagrafiche = schedaService.findComponentiGiaFamigliariBySoggettoCf(dto);

			dto.setObj(idCaso);
			dto.setObj2(csASoggetto.getCsAAnagrafica().getId());
			dto.setObj3(cf);
			
			/* Carico:
			 * 1.Relazioni del soggetto collegate al caso
			 * 2.Relazioni associate al soggetto per CF provenienti da PAI Esterni
			 * 3.Relazioni associate al soggetto per familiarità (Ulteriori soggetti) */
			
			listaRelazioniDTO = diarioService.findRelazioniFascicolo(dto);
	...
	public void carica() {
		...
		try {
			...
			Long idRelSearch = idRelazioneSelezionata;
			if(idRelazioneSelezionata == null)
				idRelSearch = listaRelazioniDTO.get(idxSelected).getRelazione().getDiarioId();
			
			//Load full
			BaseDTO dto = new BaseDTO();
			fillEnte(dto);
			dto.setObj(idRelSearch);
			dto.setObj2(idCaso);
			relazioneDTO = diarioService.findRelazioneFullById(dto);//qui sono 12 elementi! lista con chi
			idRelazioneSelezionata = null;
			
			if(relazioneDTO.getRelazione().getLstConChi() != null && !relazioneDTO.getRelazione().getLstConChi().isEmpty()){
				for(CsCDiarioConchi cs : relazioneDTO.getRelazione().getLstConChi()){
					this.lstConChiSel.add(String.valueOf(cs.getId()));
				}
			}
			
			if(relazioneDTO.getRelazione().getLstRiunioneConChi()!= null && !relazioneDTO.getRelazione().getLstRiunioneConChi().isEmpty()) {
				this.selectedRiunioneConChi.clear();
				this.selectedRiunioneConChiToView.clear();
				for(CsOSettore cs : relazioneDTO.getRelazione().getLstRiunioneConChi()){
		...
		if (listaTipiIntervento == null) {
			...
			try {
				if (fromFascicoloCartellaUtente) {
					...
					List<CsCTipoIntervento> lista = confService.findTipiInterventoSettoreCatSoc(dto);
	...
	public void aggiungiTipoInterventoButton() {
		...
		try{
			...
			if (selectedId != null) {
				...
				CsCTipoIntervento tipoInt = confService.getTipoInterventoById(dto);
				if (tipoInt != null) {
					...
					if (selectedCustomId != null) {
						...
						CsCTipoIntervento tipoIntCustom = confService.getTipoInterventoById(dto);
...
//        }
	...
	public Converter getMicroAttivitaConverter(){
		return new Converter(){
			...
			public Object getAsObject(FacesContext context,
				...
				fillEnte(dto);
				dto.setObj(id);
				return confService.getMicroAttivitaById(dto);
			}
	...
	private List<SelectItem> loadListaSettoriFlag(boolean riunioneCon) {
		...
		CeTBaseObject bo = new CeTBaseObject();
		fillEnte(bo);
		List<CsOSettore> lst = confEnteService.getSettoreAll(bo);
	...
	public List<SelectItem> getLstConChi() {
		if (this.lstConChi == null) {
			...
			SelectItem altro = null;
			BaseDTO dto = new BaseDTO();
			fillEnte(dto);
			List<KeyValueDTO> diarioConchis = confService.getDiarioConchis(dto);
			for (KeyValueDTO cs : diarioConchis) 
			{
				SelectItem item = new SelectItem(cs.getCodice(), cs.getDescrizione());
				item.setDisabled(!cs.isAbilitato());
	...
	public void conChiSelezionato(){//AjaxBehaviorEvent event
		...
		if(!lstConChiSel.isEmpty()) {
			...
			for(int i=0; i < lstConChiSel.size(); i++)
				ids.add(Long.valueOf(lstConChiSel.get(i)).longValue());
			
			BaseDTO dto = new BaseDTO();
			fillEnte(dto);
			dto.setObj(ids);
			List<CsCDiarioConchi> diarioConchi = confService.findDiarioConchisByIds(dto); 
			relazioneDTO.getRelazione().getLstConChi().addAll(diarioConchi);					
		}
	}
	...
	public Converter getConChiConverter() {
		return new Converter() {
			...
			public Object getAsObject(FacesContext context,UIComponent component, String value) {
				...
				fillEnte(dto);
				dto.setObj(id);
				return confService.findDiarioConchi(dto);
...
//					}
	...
	public void aggiungiProblematicaButton() {
		...
		if (selectedProblematicaId != null && !selectedProblematicaId.isEmpty()) {
			...
			tbProbl = confService.getProblById(dto);
		}
	...
	public List<RelazioneDTO> listaRelazioniDTO(Object var) {
		...
		if (var!=null && (var.equals("propostaAttivita")|| var.equals("propostaAttivitaExt"))) { 
			PaiBean paiBean = null;
			if(var.equals("propostaAttivita")){
				FascicoloBean fbean = (FascicoloBean) getReferencedBean("fascicoloBean");
				paiBean = fbean!=null ? fbean.getPaiBean() : null;
			}else if(var.equals("propostaAttivitaExt")){
				ProgettiIndividualiExtBean fbean = (ProgettiIndividualiExtBean) getReferencedBean("progettiInvidualiExt");
				paiBean = fbean!=null ? fbean.getPaiBean() : null;
			}
			lstRelazDToToReturn = paiBean!=null ? paiBean.getListaRelazioniDTO() : new ArrayList<RelazioneDTO>();
		} else { 
