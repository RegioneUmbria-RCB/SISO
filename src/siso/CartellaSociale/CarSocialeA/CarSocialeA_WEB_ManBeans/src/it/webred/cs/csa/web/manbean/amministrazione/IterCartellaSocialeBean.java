			...
			public void onClickCambiaStatoIter(){
				...
				try{
					for(IterSoggettoLAZY iterIesimo: listaIterSoggettoLAZY){
						...
						if(!StringUtils.isBlank(settore))
...
//							iterDto.setNomeOperatore(username);
							...
							BaseDTO dto = new BaseDTO();
				   			...
				   			for(CsASoggettoCategoriaSoc catSocLazy : listaCatSocLazy){
								...
								dto.setObj2(settore);
								boolean found = catSocService.existsRelSettoreCatsoc(dto);
								if(found == false){
							...
							iterDto.setNota("Segnalato a " + (!StringUtils.isBlank(this.settore) ? "Settore" : "Ente") + " ITER UTILITY");
							iterDto.setAttrNewStato(null);
							iterDto.setAlertUrl(null);
							iterDto.setNotificaSettoreSegnalante(true);
			...
			public void onClickValidaCF() {
				...
				while (st2.hasMoreElements()) {
					...
					for(CsASoggettoLAZY soggetto : listSoggettiCFLazy){
						...
						iterSoggLAZY.setListaCatSociali(ics.getSoggettoCategorieAttuali(dto));
						
						if(soggetto!= null){
							dto.setObj(iterSoggLAZY.getCasoId());
							iterSoggLAZY.setItStep(iterService.getLastIterStepByCaso(dto));
						}
