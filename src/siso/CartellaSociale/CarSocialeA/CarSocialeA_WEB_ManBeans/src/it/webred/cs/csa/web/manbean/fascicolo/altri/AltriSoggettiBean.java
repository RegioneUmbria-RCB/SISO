...
public class AltriSoggettiBean extends CsUiCompBaseBean implements IAltriSoggetti {
	...
	private void caricaAltriSoggetti(Object data) {
		...
		List<FamiliareDettaglio> listaFamiglia_anagrafe = new ArrayList<FamiliareDettaglio>();
		if (altriSoggetti!= null) { // mi è arrivato il dato dal parametro data
			...
			RicercaAnagraficaParams ap = new RicercaAnagraficaParams(csASoggetto.getCsAAnagrafica().getIdOrigWsTipo(),true);
			fillEnte(ap);
			ap.setIdentificativo(csASoggetto.getCsAAnagrafica().getIdOrigWsId());
			ap.setCf(csASoggetto.getCsAAnagrafica().getCf());
			
			listaFamiglia_anagrafe = CsUiCompBaseBean.getComposizioneFamiliareAllProvenienza(ap);
			famigliaComponenti = CsUiCompBaseBean.caricaParenti(csASoggetto.getAnagraficaId(), null);
		}		

		if(listaFamiglia_anagrafe!=null){
			for (FamiliareDettaglio persona : listaFamiglia_anagrafe) {
