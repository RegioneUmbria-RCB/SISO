...
public class GestioneSettoriBean extends CsUiCompBaseBean implements IGestioneSettori {
	...
	private void carica2Liv() {
		...
		try {
			...
			CeTBaseObject cet = new CeTBaseObject();
			fillEnte(cet);
			List<KeyValueDTO> lst = confService.getListaSecondoLivello(cet);
			this.lst2Liv = this.convertiLista(lst);
...
/*	public void onChangeEnteEsterno(){
	...
	private void caricaOrganizzazioni() {
		...
		try {
			...
			CeTBaseObject cet = new CeTBaseObject();
			fillEnte(cet);
			lstOrganizzazioni = confEnteService.getOrganizzazioniAll(cet);
	...
	public void caricaSettori() {
		...
		if(selectedOrganizzazioni != null && !selectedOrganizzazioni.isEmpty()) {
			...
			try {
				lstSettori = confEnteService.findSettoreDTOByOrganizzazione(dto);
			} catch(Exception e) {
				addErrorFromProperties("caricamento.error");
				logger.error(e.getMessage(),e);
			}
			...
			try {
				...
				for(CsOOrganizzazione org: selectedOrganizzazioni) {
					if(!"1".equals(org.getAbilitato())) {
						org.setAbilitato("1");
						dto.setObj(org);
						confEnteService.updateOrganizzazione(dto);
					}
				}
		...
		if(selectedOrganizzazioni != null) {
			try {
				...
				for(CsOOrganizzazione org: selectedOrganizzazioni) {
					...
					try {
						confEnteService.eliminaOrganizzazione(dto);
					} catch(Exception e) {
						ok = false;
					}
			...
			try {
				...
				if(!this.enteAccessoAM){
					...
					if(newOrganizzazione.getCodRouting()!=null){
						...
						CsOOrganizzazione o = confEnteService.getOrganizzazioneByCodFittizio(dto);
						if(o!=null){
				...
				confEnteService.salvaOrganizzazione(dto);
	...
	public void aggiungiSettore() {
		...
		if(!StringUtils.isBlank(newSettore.getNome())) {
			try {
				...
				confEnteService.salvaSettore(dto);
		...
		if(selectedSettori != null) {
			try {
				...
				for(SettoreCatSocialeDTO cs: selectedSettori) {
					...
					confEnteService.updateSettore(dto);
				}
	...
	public void eliminaSettori() {
		...
		if(selectedSettori != null) {
			try {
				...
				for(SettoreCatSocialeDTO cs: selectedSettori) {
					dto.setObj(cs.getSettore());
					confEnteService.eliminaSettore(dto);
				}
