...
public class LazyListaFseModel extends LazyDataModel<ListaFseDTO>  {
	...
	public List<ListaFseDTO> load(int first, int pageSize, String sortField,SortOrder sortOrder, Map filters) {
		...
		if(canViewLista) {
			...
			boolean canViewListaAmbito = CsUiCompBaseBean.checkPermesso(PermessiCartella.ITEM, PermessiCartella.VISUALIZZAZIONE_LISTA_FSE_ZS);
			
			if(!canViewListaAmbito)
			...
			Long[] criteriaTipo = (Long[]) CsUiCompBaseBean.getSession().getAttribute(FiltroFse.TIPO);
			if (criteriaTipo!=null){
				searchCriteria.setTipoFse(Arrays.asList(criteriaTipo));
			}
			...
			Boolean isOnlyFirstPor = (Boolean) CsUiCompBaseBean.getSession().getAttribute(FiltroFse.EXTRACT_FIRST);
			searchCriteria.setOnlyFirstByCFProgetto(isOnlyFirstPor);
