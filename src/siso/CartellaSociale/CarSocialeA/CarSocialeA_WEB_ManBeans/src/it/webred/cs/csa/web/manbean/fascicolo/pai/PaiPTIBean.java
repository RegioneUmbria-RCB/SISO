		...
		try {
			if (listaCaseManager == null || listaCaseManager.isEmpty()) {
				...
				if (settoreId != null) {
					...
					List<KeyValueDTO> lstOpSettAnagrafica = confEnteService.findOperatoreIdAnagraficaBySettore(oDto);
...
//		else
	...
	private void addAlertUpdPTI( CsPaiMastSoggDTO minore, String tipoPTIAlert, String tipoOperazione) throws Exception {
		...
		if (tipoPTIAlert.equalsIgnoreCase("CASE_MANAGER")) {
			...
			opSett = confEnteService.findOperatoreSettoreById(odto);
			...
			fillEnte(dto2);
			csSOSettore = confEnteService.findSettoreByRelStruttura(dto2);	
