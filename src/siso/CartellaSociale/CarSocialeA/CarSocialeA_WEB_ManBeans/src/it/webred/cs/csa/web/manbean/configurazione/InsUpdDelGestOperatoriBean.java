...
public class InsUpdDelGestOperatoriBean extends CsUiCompBaseBean implements Serializable {
	...
	protected void loadOrganizzazioni() {
		...
		try {
			...
			fillEnte(cet);
			organizzazioniBean = confEnteService.getOrganizzazioni(cet);
	...
	public void onSelOrganizzazione() {
		...
		try {
			...
			fillEnte(opDto);
			List<KeyValueDTO> beanLst = confEnteService.findSettoriByOrganizzazione(opDto);
			settori = convertiLista(beanLst);
			...
			CsOOrganizzazione org = confEnteService.getOrganizzazioneById(bdto);
	...
	public void onSelSettore() {
		...
		try {
			...
			if(operatoreSelezionato!=null && operatoreSelezionato.getIdOperatore()!=null){
				...
				fillEnte(opDto);		
				CsOOperatoreSettore opSet = confEnteService.findOperatoreSettoreById(opDto);			
				if (opSet != null) {
					opDto.setIdOperatoreSettore(opSet.getId());
					CsOOperatoreTipoOperatore opTipo = confEnteService.getTipoByOperatoreSettore(opDto);
				...
				if (amGroup != null && !amGroup.equals("")) {
					String[] arr = amGroup.split(DataModelCostanti.SEPARATORE_AM_GROUP);
					if (arr.length > 0) {
	...
	private void loadRuoli(String belfiore) throws Exception {
		...
		if(belfiore!=null){
		    ...
		    fillEnte(dto);
		    dto.setObj(belfiore);
			hmRuoli = confEnteService.getCodificaRuoli(dto);
		}else{
			CeTBaseObject cet = new CeTBaseObject();
			fillEnte(cet);
			/*Recupero le organizzazioni configurate per il datarouting, quindi aventi BELFIORE valorizzato*/
			List<CsOOrganizzazione> orgs = confEnteService.getOrganizzazioniAccesso(cet);
	...
	public List<CsOOperatoreSettoreEstesa> getConfigurazioniUtente(Long idOp){
		...
		if (idOp != null) {
			try {
				...
				fillEnte(opDto);
				lst = confEnteService.findOperatoreSettoreEstesaByOperatore(opDto);
			} catch (Exception e) {
				addErrorFromProperties("caricamento.error");
				logger.error(e.getMessage(),e);
			}	
