	...
	public String createQueryListaCasiAssegnati(boolean count){
		...
		if(count)
			sql += "SELECT count(DISTINCT ANAGRAFICA_ID) ";
	            ...
	            "DATA_CREAZIONE, " +
	            "CFG_IT_STATO_ID, "+
	            "decode(intc.id, null,  0, 1) exists_interventi"; 
		...
		sql += " FROM (" + sqlCasi + " ) U "
				...
				+ " AND IND.TIPO_INDIRIZZO_ID = "+TipoIndirizzo.RESIDENZA_ID
				...
				+"   LEFT JOIN CS_VISTA_INTERVENTI_CASO intc on (intc.caso_id = u.caso_id) ";
