...
public class ParentiDAO extends CarSocialeBaseDAO implements Serializable {
	...
	@SuppressWarnings("unchecked")
	public List<CsAFamigliaGruppoGit> getFamiglieGruppoGit(String provenienza) {
		List<CsAFamigliaGruppoGit> lista = new ArrayList<CsAFamigliaGruppoGit>();
		try{	
			Query q = em.createNamedQuery("CsAFamigliaGruppoGit.getFamiglieGruppoGitByProvenienza")
	...
	public int deleteComponenteGit(List<BigDecimal> rimossi) {
		int num = 0;
		try{
			if(!rimossi.isEmpty()){
				Query q = em.createNamedQuery("CsAComponenteGit.deleteByIds");
				q.setParameter("ids", rimossi);
				num = q.executeUpdate();
			}
		}catch(Exception e){
			logger.error(e.getMessage(),e);
	...
	public CsAComponente findComponenteById(Long componenteId) {
