...
import it.webred.cs.csa.ejb.queryBuilder.SchedaUdcQueryBuilder;
...
public class SchedaSegrDAO extends CarSocialeBaseDAO implements Serializable {
	...
	public List<CsSsSchedaSegr> findSchedaSegrByIdAnagraficaNotSS(Long idAnagrafica) {
		...
		q.setParameter("idAnagrafica", idAnagrafica);
		q.setParameter("provenienza", DataModelCostanti.SchedaSegr.PROVENIENZA_SS);
	...
	public List<DatiSchedaAccessoDTO> getSchedeSegr(SchedaSegrDTO criteria) throws Throwable {
		...
		try{
			SchedaUdcQueryBuilder qb = new SchedaUdcQueryBuilder(criteria);
			String sql = qb.getQuery(false);
			Integer first= criteria.getFirst();
			int pageSize= criteria.getPageSize();
			logger.debug("ListaSchedeSegretariato - SQL["+sql+"]");
			Query q = em.createNativeQuery(sql);
			
			String params = qb.setFilterParameters(q);
			logger.debug("ListaSchedeSegretariato - PARAMS["+params+"]");
	...
	public Integer getSchedeSegrCount(boolean onlyNew,SchedaSegrDTO criteria) throws Throwable{
		...
		try{
			SchedaUdcQueryBuilder qb = new SchedaUdcQueryBuilder(criteria);
			String sql = qb.getQuery(true);
			logger.debug("getSchedeSegrCount SQL["+sql+"]");
			Query q = em.createNativeQuery(sql);
