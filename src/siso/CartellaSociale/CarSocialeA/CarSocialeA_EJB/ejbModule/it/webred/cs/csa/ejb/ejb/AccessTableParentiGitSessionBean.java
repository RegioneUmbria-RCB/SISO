...
public class AccessTableParentiGitSessionBean extends CarSocialeBaseSessionBean implements AccessTableParentiGitSessionBeanRemote {
	...
	public InfoRecapitiDTO findInfoRecapiti(BaseDTO dto){
		...
		if(s!=null){
			...
			CsAAnaIndirizzo residenza = indirizzoDAO.getResidenzaBySoggetto(s.getAnagraficaId());
			if(residenza!=null){
				i.setResIndirizzo(residenza.getIndirizzo());
				i.setResCivico(residenza.getCivicoNumero());
				i.setResComune(new KeyValueDTO(residenza.getComCod(), residenza.getComDes()));
				i.setResStato( new KeyValueDTO(residenza.getStatoCod(), residenza.getStatoDes()));
			} 
		}else
