...
public class AccessTableDatiPorSessionBean extends CarSocialeBaseSessionBean implements AccessTableDatiPorSessionBeanRemote {
	...
	public CsExtraFseDatiLavoro findDatiPorUdcBySchedaId(BaseDTO dto) {
		return this.dao.findDatiPorUdcBySchedaId((Long) dto.getObj());
	}
	
	@Override
	public CsExtraFseDatiLavoro saveDatiPor(BaseDTO dto) {
		CsExtraFseDatiLavoro lavoro = (CsExtraFseDatiLavoro) dto.getObj();
		lavoro = this.dao.fillDatiLavoroPor(lavoro, dto.getUserId());
		return this.dao.saveDatiPor(lavoro);
	}
	
	@Override
	public void eliminaDatiPor(BaseDTO dto) {
		this.dao.eliminaDatiPor((Long) dto.getObj());
	}
	...
	public SiruResultDTO validaSiru(BaseDTO dto) {
		try {
			...
			if (spd.getIdTitoloStudio() != null) {
				SiruDominioDTO sd = this.dominiSiruService.findByIdSiso(SiruEnum.TITOLO_STUDIO.name(), spd.getIdTitoloStudio());
				if (sd == null) {
					msg.add("Titolo di studio" + msgSiru);
				} else {
					siru.setTitoloStudio(sd.getCodiceSiru());
				}
			}
			...
			if (ingMercato != null && !StringUtils.isBlank(ingMercato.getId()) && !StringUtils.isBlank(ingMercato.getDescrizione())) {
				...
				SiruDominioDTO sd = this.dominiSiruService.findByIdSiso(SiruEnum.CONDIZIONE_MERCATO.name(), spd.getCsTbIngMercato().getId());
				if (sd == null) {
					msg.add("Condizione di ingresso nel mondo del lavoro" + msgSiru);
				} else {
					siru.setCondMercatoIngresso(sd.getCodiceSiru());
				}
			...
			if (mappaConf.getAzCodAteco().isAbilitato()) {
				if (!StringUtils.isBlank(spd.getAzCodAteco())) {
					SiruDominioDTO sisoAteco = this.dominiSiruService.findById(SiruEnum.SISO_ATECO.name(),spd.getAzCodAteco());
					SiruDominioDTO sd = this.dominiSiruService.findById(SiruEnum.ATECO.name(), sisoAteco.getCodiceSiru() + "_" + sisoAteco.getVersione());
					
					if (sd == null) {
						msg.add("Ateco" + msgSiru);
					} else {
						siru.setCodAtecoAnno(sd.getCodiceSiru());
					}
			...
			if (mappaConf.getAzDimensione().isAbilitato()) {
				if (!StringUtils.isBlank(spd.getDescDimAzienda())) {
					SiruDominioDTO sd = this.dominiSiruService.findByIdSiso(SiruEnum.DIMENSIONE_AZIENDA.name(), spd.getDescDimAzienda());
					if (sd == null) {
						msg.add("Dimensione azienda" + msgSiru);
					} else {
						siru.setDimensioneAziendaId(sd.getCodiceSiru());
					}
			...
			if (mappaConf.getAzFormaGiuridica().isAbilitato()) {
				if (!StringUtils.isBlank(spd.getAzFormaGiuridica())) {
					...
					if (sd == null) {
						msg.add("Forma giuridica" + msgSiru);
					} else {
						siru.setFrmGiuridicaCod(sd.getCodiceSiru());
					}
			...
			if (!StringUtils.isBlank(spd.getGrpVulnerabilita())) {
				SiruDominioDTO sd = this.dominiSiruService.findByIdSiso(SiruEnum.GRUPPO_VUL_PART.name(), spd.getGrpVulnerabilita());
				if (sd == null) {
					msg.add("Gruppo vulnerabile" + msgSiru);
				} else {
					siru.setCodVulnerabilePa(sd.getCodiceSiru());
				}
			}
			...
			if (mappaConf.getLavoroOrario().isAbilitato()) {
				if (!StringUtils.isBlank(spd.getDescOrarioLavoro())) {
					SiruDominioDTO sd = this.dominiSiruService.findByIdSiso(SiruEnum.TIPO_ORARIO_LAVORO.name(), spd.getDescOrarioLavoro());
			...
			if (mappaConf.getLavoroTipo().isAbilitato()) {
				if (!StringUtils.isBlank(spd.getDescTipoLavoro())) {
					SiruDominioDTO sd = this.dominiSiruService.findByIdSiso(SiruEnum.TIPOLOGIA_LAVORO.name(), spd.getDescTipoLavoro());
					if (sd == null) {
						msg.add("Tipologia di lavoro" + msgSiru);
					} else {
						siru.setTipologiaLavoroId(sd.getCodiceSiru());
					}
			...
			if (mappaConf.getDurataRicercaLavoro().isAbilitato()) {
				if (!StringUtils.isBlank(spd.getDurataRicLavoroId())) {
					SiruDominioDTO sd = this.dominiSiruService.findByIdSiso(SiruEnum.DURATA_RICERCA.name(), spd.getDurataRicLavoroId());
					
					if (sd == null) {
						msg.add("Durata di ricerca" + msgSiru);
					} else {
						siru.setDurataRicerca(sd.getCodiceSiru());
					}
			...
			if (!StringUtils.isBlank(spd.getCittadinanza())) {
				...
				SiruDominioDTO sd = this.dominiSiruService.findByDesc(SiruEnum.AM_NAZIONALITA.name(), spd.getCittadinanza());
				// ITALIA da 1 a -> 0000
				if (sd != null && sd.getCodiceSiru().equals("1")) {
					sd.setCodiceSiru("000");
				}
				// Verifico che sia mappato correttamente in FSE_CHK_CITTADINANZA
				String codIstatCittadinanza = sd.getCodiceSiru();
				sd = this.dominiSiruService.findById(SiruEnum.CITTADINANZA.name(), codIstatCittadinanza);
				if (sd == null) {
					msg.add("Cittadinanza [" + spd.getCittadinanza() + "] cod.Istat[" + codIstatCittadinanza + "]" + msgSiru);
				} else {
					siru.setCittadinanza(sd.getCodiceSiru());
				}
			...
			if (!StringUtils.isBlank(spd.getComuneNascitaCod())) {
				...
				SiruDominioDTO sd = this.dominiSiruService.findById(SiruEnum.IT_COMUNE.name(), spd.getComuneNascitaCod());
				if (sd != null) {
					siru.setComuneNascitaId(sd.getCodiceSiru());
				} else {
					msg.add("Comune di nascita" + msgSiru);
				}
				...
				SiruDominioDTO sd = this.dominiSiruService.findByIdSiso(SiruEnum.IT_NAZIONE_VIEW.name(), spd.getStatoNascitaCod());
	
				if (sd == null) {
					msg.add("Nazione di nascita" + msgSiru);
				} else {
					siru.setNazioneNascitaId(sd.getCodiceSiru());
				}
			...
			if (mappaConf.getAzComune().isAbilitato()) {
				if (!StringUtils.isBlank(spd.getAzComuneCod())) { // !regMarche &&
					SiruDominioDTO sd = this.dominiSiruService.findById(SiruEnum.LOC_GEOG_ATTUALE.name(), spd.getAzComuneCod());
					if (sd != null) {
						siru.setComuneAziendaId(sd.getCodiceSiru());
					} else {
						msg.add("Comune azienda" + msgSiru);
					}
			...
			if (mappaConf.getPagResDom().isAbilitato()) {
				if (!StringUtils.isBlank(spd.getFlagResDom())) {
					SiruDominioDTO sd = this.dominiSiruService.findByIdSiso(SiruEnum.RESIDENZA.name(), spd.getFlagResDom());
					if (sd != null) {
						siru.setFlagResidenza(new Integer(sd.getCodiceSiru()));
					} else {
						msg.add("Flag Residenza" + msgSiru);
					}
				}
			}
			...
			if (!StringUtils.isBlank(spd.getCodIstatComuneResidenza())) {
				SiruDominioDTO sd = this.dominiSiruService.findById(SiruEnum.LOC_GEOG_ATTUALE.name(), spd.getCodIstatComuneResidenza());
				if (sd != null)
			...
			if (!StringUtils.isBlank(spd.getCodIstatComuneDomicilio())) {
				SiruDominioDTO sd = this.dominiSiruService.findById(SiruEnum.LOC_GEOG_ATTUALE.name(), spd.getCodIstatComuneDomicilio());
				if (sd != null)
	...
	public List<String> validaStampa(BaseDTO dto) {
		...
		else if (DataModelCostanti.NON_RILEVATO.equalsIgnoreCase(fse.getTitoloStudio()))
		...
		else if (DataModelCostanti.NON_RILEVATO.equalsIgnoreCase(fse.getCondLavoro()) )
