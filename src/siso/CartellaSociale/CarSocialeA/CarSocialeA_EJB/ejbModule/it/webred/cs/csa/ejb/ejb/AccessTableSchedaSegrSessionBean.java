...
public class AccessTableSchedaSegrSessionBean extends CarSocialeBaseSessionBean implements AccessTableSchedaSegrSessionBeanRemote  {
	...
	public List<ListaDatiSchedaUdC> getListaSchedeUdC(SchedaSegrDTO dto) throws Throwable {	
		List<ListaDatiSchedaUdC> lstOut = new ArrayList<ListaDatiSchedaUdC>();
		List<DatiSchedaAccessoDTO> lstIn = schedaSegrDao.getSchedeSegr(dto);
		for(DatiSchedaAccessoDTO sa : lstIn){
			ListaDatiSchedaUdC out = new ListaDatiSchedaUdC();
			...
			if(dto.isLoadStatoIterCartella()){
				if (sa.getCasoId()!= null && sa.getCasoId().longValue()>0) {
					BaseDTO itDto = new BaseDTO();
					copiaCsTBaseObject(dto, itDto);
					itDto.setObj(sa.getCasoId());
					CsIterStepByCasoDTO itStep = iterSessionBean.getLastIterStepByCasoDTO(itDto);
					out.setIterCaso(itStep);
				}	
			}
			out.setUdc(sa);
			lstOut.add(out);
