...
public class AlertDAO extends CarSocialeBaseDAO implements Serializable {
	...
	public List<CsAlertBASIC> getAlerts(Long idOpSettore, Long idSettore, Long idOrganizzazione, List<String> listaTipo, Boolean visibile) throws Exception {
		...
		if(idOpSettore!=null || idSettore!=null || idOrganizzazione!=null){
			where+= " AND ( 1=0 ";
		...
		logger.debug("getAlerts idOperatoreSettore["+idOpSettore+"] idSettore["+idSettore+"] idOrganizzazione["+idOrganizzazione+"] tipi["+listaTipo+"]" );
		...
		try{
			...
			if(idOpSettore != null) 
				q.setParameter("idOperatore", idOpSettore);
