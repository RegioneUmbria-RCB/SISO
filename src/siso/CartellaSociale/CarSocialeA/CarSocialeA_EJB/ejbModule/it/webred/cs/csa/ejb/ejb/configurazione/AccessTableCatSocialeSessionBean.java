...
public class AccessTableCatSocialeSessionBean extends CarSocialeBaseSessionBean implements AccessTableCatSocialeSessionBeanRemote  {
	...
	public Boolean existsRelSettoreCatsoc(BaseDTO dto){
		...
		Long idSettore = (Long)dto.getObj2();
		CsRelSettoreCatsoc rel = catSocialeDao.findRelSettoreCatsocById(idCatSoc, idSettore);
		return rel!=null;
	}
	...
	public Boolean isCategoriaSocialeSettore(BaseDTO dto){
		...
		Long settoreId = (Long)dto.getObj();
		Long categoriaId = (Long)dto.getObj2();
		List<CsRelSettoreCatsoc> lista = catSocialeDao.findRelSettoreCatsocBySettore(settoreId);
		Iterator<CsRelSettoreCatsoc> it = lista.iterator();
		while(it.hasNext() && !matchCatSoc){
			CsRelSettoreCatsoc rel = it.next();
			if(rel.getAbilitato() && rel.getCsCCategoriaSociale().getAbilitato() && categoriaId.equals(rel.getCsCCategoriaSociale().getId()))
				matchCatSoc=true;
		}
		return matchCatSoc;
	}
