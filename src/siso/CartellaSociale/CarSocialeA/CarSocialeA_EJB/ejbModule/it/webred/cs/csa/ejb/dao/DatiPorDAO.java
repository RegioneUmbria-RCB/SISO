...
public class DatiPorDAO extends CarSocialeBaseDAO implements Serializable {
	...
	public List<KeyValueDTO> getListaComuniResidenzaUsati() {
		...
		try {
			String sql = "select distinct v.residenzaComuneId, v.residenzaComuneDesc " + "from CsVistaFse v " + "order by residenzaComuneDesc";
	...
	public List<ListaFseDTO> getListaFse(FseSearchCriteria searchCriteria) {
		...
		try {
			...
			Query q = em.createNativeQuery(sql, CsVistaFse.class);
			...
			for (CsVistaFse o : res) {
				...
				fse.setEnteTitolare(o.getOrganizzazioneTitolareDesc());
	 ...
	 * @lastUpdate 2025-10-28 - DDV
	 */
	public CsExtraFseDatiLavoro fillDatiLavoroPor(CsExtraFseDatiLavoro csExtraFseDatiLavoro, String username) {
		if (csExtraFseDatiLavoro != null) {

			if (csExtraFseDatiLavoro.getId() == null) {
				csExtraFseDatiLavoro.getMaster().setDtIns(new Date());
				csExtraFseDatiLavoro.getMaster().setUsrIns(username);
				csExtraFseDatiLavoro.setUserIns(username);
				csExtraFseDatiLavoro.setDtIns(new Date());
			} else {
				csExtraFseDatiLavoro.setUserMod(username);
				csExtraFseDatiLavoro.setDtMod(new Date());
				csExtraFseDatiLavoro.getMaster().setUsrMod(username);
				csExtraFseDatiLavoro.getMaster().setDtMod(new Date());
			}

			if (csExtraFseDatiLavoro.getMaster().getSiru() != null) {
				Long masterId = csExtraFseDatiLavoro.getMaster().getId();
				if (masterId == null) {
					csExtraFseDatiLavoro.getMaster().getSiru().setMaster(csExtraFseDatiLavoro.getMaster());
				} else {
					csExtraFseDatiLavoro.getMaster().getSiru().setId(masterId);
					csExtraFseDatiLavoro.getMaster().getSiru().setMaster(csExtraFseDatiLavoro.getMaster());
				}
			}

			// Se non è PDV il nuovo campo va a NULL
			if (csExtraFseDatiLavoro.getProgetto() == null || csExtraFseDatiLavoro.getProgetto().getDescrizione() == null
					|| !csExtraFseDatiLavoro.getProgetto().getDescrizione().contains("PDV")) {
				csExtraFseDatiLavoro.setProgettoVitaValore(null);
			}
			
			// Se non sono un FSE tutti i campi vanno a NULL
			if (csExtraFseDatiLavoro.getProgetto() == null || csExtraFseDatiLavoro.getProgetto().getDescrizione() == null
					|| !csExtraFseDatiLavoro.getProgetto().getDescrizione().contains("FSE")) {
				csExtraFseDatiLavoro.setAzCodAteco(null);
				csExtraFseDatiLavoro.setDescDimAzienda(null);
				csExtraFseDatiLavoro.setAzFormaGiuridica(null);
				csExtraFseDatiLavoro.setDescOrarioLavoro(null);
				csExtraFseDatiLavoro.setDescTipoLavoro(null);
				csExtraFseDatiLavoro.setAzPi(null);
				csExtraFseDatiLavoro.setAzCf(null);
				csExtraFseDatiLavoro.setAzRagioneSociale(null);
				csExtraFseDatiLavoro.setAzVia(null);
				csExtraFseDatiLavoro.setDurataRicLavoroId(null);
				csExtraFseDatiLavoro.setAzComuneCod(null);
				csExtraFseDatiLavoro.setAnnoConseguimentoTitoloStu(null);
				csExtraFseDatiLavoro.setIban(null);
				csExtraFseDatiLavoro.setFlagResDom(null);
				csExtraFseDatiLavoro.setComunicaVul(null);
				csExtraFseDatiLavoro.setDtSottoscrizione(null);
				csExtraFseDatiLavoro.setSoggettoAttuatore(null);
				csExtraFseDatiLavoro.setProgettoAttivita(null);
			}
			
		}

		return csExtraFseDatiLavoro;
	}
