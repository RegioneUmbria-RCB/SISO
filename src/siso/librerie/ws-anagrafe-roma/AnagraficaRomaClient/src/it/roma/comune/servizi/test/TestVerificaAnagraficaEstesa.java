...
public class TestVerificaAnagraficaEstesa {

	NVASoap soap;
	private String esito = "OK";
	
	public TestVerificaAnagraficaEstesa(NVASoap soap){
		this.soap=soap;
	}
	public void doTest(String tipoStr, String cognome, String nome, String sesso, String annoIniziale,String annoFinale   ){
		VerificaAnagraficaEstesa va= new VerificaAnagraficaEstesa();
		...
		if(annoIniziale != null && !annoIniziale.equals("-"))
			va.setAnnoIniziale(Short.parseShort(annoIniziale));
		if(annoFinale != null && !annoFinale.equals("-"))
			va.setAnnoFinale(Short.parseShort(annoFinale));
	 
		 
	    LogHeader header= new LogHeader();
		
		header.setLogGuid(TestMain.getUID());
		VerificaAnagraficaEstesaResponse respEstesa=null;
		try {
			respEstesa=soap.verificaAnagraficaEstesa(va, header);
			ArrayList  arr_persona = new ArrayList();
			VerificaAnagraficaEstesaResponseVerificaAnagraficaEstesaResult  rEstesa=  respEstesa.getVerificaAnagraficaEstesaResult();

			for(int i=0; i< rEstesa.get_any().length; i++ ){
				 
				 System.out.println(rEstesa.get_any()[i].toString());
				 System.out.println("-----------------------------------");
			}
//				for(int i=0; i< rEstesa.get_any().length; i++ ){
//					 System.out.println(rEstesa.get_any()[i].toString());
//					 NodeList nListM = 	rEstesa.get_any()[i].getElementsByTagName("Messaggio");
//					 for (int temp = 0; temp < nListM.getLength(); temp++) {
//						 esito = elaboraXMLMessaggio(nListM.item(i));
//	 
//	 				}
//					 
//					 NodeList nList = 	rEstesa.get_any()[i].getElementsByTagName("Persona");
//						
...
//			    }
	...
	public ArrayList doTestDebug() throws Exception{
		...
		try{
			...
			MessageElement[] me =  convertXMLStringtoMessageElement("./resources/ElencoEstesoPersone.xml");
