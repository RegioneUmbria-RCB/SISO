...
public class InterventoErogazAttrBean implements Serializable {
	...
	protected CsCfgAttrUnitaMisura getAttrUnitaMisuraSelezionata()
	{
		...
		try{
			AccessTableConfigurazioneSessionBeanRemote confService = (AccessTableConfigurazioneSessionBeanRemote)  CsUiCompBaseBean.getCarSocialeEjb( "AccessTableConfigurazioneSessionBean");
			
			BaseDTO dto = new BaseDTO();
			CsUiCompBaseBean.fillEnte(dto);
			dto.setObj(csAttr.getId());
			dto.setObj2(um!=null  ? um.getId() : null);
			return confService.findAttrUnitaMisura(dto);
