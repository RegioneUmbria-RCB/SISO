...
import it.webred.cs.json.barthel.JsonBarthelBaseDataBean;
...
public class JsonBarthelMainDataBean extends JsonBarthelBaseDataBean{
...
/*	protected List<SelectItem> CreateOptionList( String baseClassName, StringIntPairBean... args )
	{
		...
		if( args != null )
		{
			for( int i = 0; i < args.length; i++)
			{
				...
				mapDescrizioni.put(className, pair.getKey());
			}
		}
		return retList;
	}*/
	...
	public String getPunteggioTotaleLegenda(){
		String label = "";
		if(punteggioTotale!=null){
			if(punteggioTotale>=100)
				return "Autosufficiente";
			else if(punteggioTotale<100 && punteggioTotale>=91 )
				return "Quasi autosufficiente";
			else if(punteggioTotale<=90 && punteggioTotale>=75 )
				return "Dipendenza lieve";
			else if(punteggioTotale<=74 && punteggioTotale>=50 )
				return "Dipendenza moderata";
			else if(punteggioTotale<=49 && punteggioTotale>=25 )
				return "Dipendenza grava";
			else if(punteggioTotale<24)
				return "Dipendenza completa";
		}	
		return label;
	}
	...
	public List<String> checkObbligatorieta() {
		...
		if( punteggioTotale <= 5.0 && (numeroPannoloni == null || numeroPannoloni <= 0) )
			messagges.add("- non hai indicato il numero di pannolini");
		
		if ( StringUtils.isEmpty( alimentazione ) ) messagges.add("- Alimentazione");
		if ( StringUtils.isEmpty( bagnoDoccia) ) messagges.add("- Bagno/doccia (lavarsi)");
		if ( StringUtils.isEmpty( igienePersonale) ) messagges.add("- Igiene personale");
		if ( StringUtils.isEmpty( abbigliamento) ) messagges.add("- Abbigliamento");
		if ( StringUtils.isEmpty( continenzaIntestinale) ) messagges.add("- Incontinenza intestinale");
		if ( StringUtils.isEmpty( continenzaUrinaria) ) messagges.add("- Incontinenza urinaria");
		if ( StringUtils.isEmpty( usoGabinetto) ) messagges.add("- Uso del gabinetto");					
		if ( StringUtils.isEmpty( lettoCarrozzina ) )	messagges.add("- Trasferimento letto-carrozzina");
		if ( StringUtils.isEmpty( deambulazione) ) messagges.add("- Deambulazione");
		if ( StringUtils.isEmpty( scale) ) messagges.add("- Scale");
		if ( StringUtils.isEmpty( usoCarrozzina) ) messagges.add("- Uso della carrozzina");
		if ( StringUtils.isEmpty( intervistaA ) ) messagges.add("- Chi hai intervistato per redigere la scheda Barthel");

		
		if ( StringUtils.isNotEmpty( altraProtesiDaChi ) && !"Nessuno".equals( altraProtesiDaChi ) )
			if( StringUtils.isEmpty(altraProtesi)) messagges.add("- non hai indicato 'altro da specificare' nella sezione protesi");
	...
	public String fillReportAusili(){
		String s = "";
		s+= "<b>Deambulatore:</b> "+this.deambulatore;
		s+= "<br/><b>Carrozzina:</b> "+this.carrozzina;
		s+= "<br/><b>Materassino:</b> "+this.materassinoCuscinoAntidecubito;
		s+= "<br/><b>Letto ortopedico:</b> "+this.lettoOrtopedico;
		s+= "<br/><b>Presidio per incontinenza:</b> "+this.presidioPerIncontinenza;
		s+= this.altraProtesi!=null ? ("<br/><b>"+this.altraProtesi+":</b> "+this.altraProtesiDaChi):"";
		return s;		
	}
}
