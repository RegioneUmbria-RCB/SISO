...
public abstract class DatiValGestioneMan extends CsUiCompBaseBean implements IDatiValiditaGestione {
	...
	public List<ValiditaDTO> riversaLista(){
		...
		for(ValiditaCompBaseBean comp: getLstComponents()) {
			...
			cs.setId(comp.getId());
			cs.setDescrizione(comp.getDescrizione());
			lista.add(cs);
		}
		return lista;
	}
	
	public void caricaLista(List<ValiditaDTO> lista){
		List<ValiditaCompBaseBean> listaComp = new ArrayList<ValiditaCompBaseBean>();
		for(ValiditaDTO val : lista){
			ValiditaCompBaseBean comp = new ValiditaCompBaseBean();
			comp.setDataFine(val.getDataFine());
			comp.setDataInizio(val.getDataInizio());
			comp.setDescrizione(val.getDescrizione());
			comp.setId(val.getId());
			addDettagliSelezione(comp);
			listaComp.add(comp);
		}
		setLstComponents(listaComp);
	}
