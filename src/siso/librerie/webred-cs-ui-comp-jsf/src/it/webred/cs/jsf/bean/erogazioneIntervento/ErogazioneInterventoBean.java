...
public class ErogazioneInterventoBean extends CsUiCompBaseBean implements Serializable {
	...
	private String tipoSoggettoBeneficiario = StringUtils.EMPTY;
	...
	// INIZIO: SISO-2448 - Modifica erogazioni per GRUPPO
	private CsTbTipoSgtBeneficiario selectedTipoSgtBeneficiario;
	public boolean disableInputCodiceFiscale = true;
	public String inputRagioneSociale;
	public String inputCodiceFiscale;
	...
	public String getInputRagioneSociale() {
		return inputRagioneSociale;
	}

	public void setInputRagioneSociale(String inputRagioneSociale) {
	...
	public void setInputCodiceFiscale(String inputCodiceFiscale) {
		this.inputCodiceFiscale = inputCodiceFiscale;
	}

	public boolean isDisableInputCodiceFiscale() {
		return disableInputCodiceFiscale;
	}

	public void setDisableInputCodiceFiscale(boolean disableInputCodiceFiscale) {
		this.disableInputCodiceFiscale = disableInputCodiceFiscale;
	}

	public CsTbTipoSgtBeneficiario getSelectedTipoSgtBeneficiario() {
		return selectedTipoSgtBeneficiario;
	}

	public void setSelectedTipoSgtBeneficiario(CsTbTipoSgtBeneficiario selectedTipoSgtBeneficiario) {
		this.selectedTipoSgtBeneficiario = selectedTipoSgtBeneficiario;
	}
	...
	// FINE: SISO-2448 - Modifica erogazioni per GRUPPO
	
	// #ROMACAPITALE fine
	public void inizializzaNuovaErogazione(Long tipoInterventoId, Long tipoInterventoCustomId, SoggettoErogazioneBean soggettoErogazione, Long catSocialeId,
			boolean dentroFascicolo) {
		...
		this.setCsIPs(new CsIPs()); // MOD-RL
		this.testataDisabled = false;
		...
		this.initTipoInterventoCustom(tipoInterventoCustomId);

		this.buildInterventoErogazzAttList(tipoInterventoCustomId != null ? tipoInterventoCustomId : tipoInterventoId);
		this.loadListaStatiInterventi(tipoInterventoCustomId != null ? tipoInterventoCustomId : tipoInterventoId);
		//loadListaSettori();
		this.resetSettoriTestata();
		this.resetOrganizAndNuovoIntEseg();

		this.nuovoIntEsegMast.setTipoBeneficiario(DataModelCostanti.ListaBeneficiari.INDIVIDUALE);
		this.nuovoIntEsegMast.setTotBeneficiari(Integer.valueOf(1));

		// this.codFinanziamento=null;

		// Recupera oggetto intervento
		this.erogazioneHistory = new InterventoErogazHistoryBean();
		this.interEsegHistory = this.erogazioneHistory.initialize
				( this.mappaErogStatoAttr
				, this.tipoIntervento
				, this.tipoIntCustom
				, catSocialeId
				);

		this.resetBoolDisabledSpese();
		...
		this.initDatiSituazioneEconomicaVerificata(tipoInterventoId);

		//SISO-962 INIZIO
		this.initDatiResidenza();
		this.initDatiNascita();
		//SISO-962 FINE
		
		this.tipoSoggettoBeneficiario = "";
		this.inputRagioneSociale = "";
		this.inputCodiceFiscale = "";
	}
	...
	public void caricaListaIdSettoriByIntervento(Long idInterventoISTAT, Long idInterventoCUSTOM) {
		this.setVisualizzaPannelloDettaglio(false);
		...
		this.listaIdSettoriByIntervervento = confEnteService.findIdSettoriByInterventoISTATandInterventoCustom(dto);
	...
	private void initDatiNascita() {
		...
		if (StringUtils.isBlank(this.soggettoErogazione.getComuneNascita()) && StringUtils.isBlank(this.soggettoErogazione.getNazioneNascita())) {

			CsASoggettoLAZY csASoggetto = (this.soggettoErogazione.getCsASoggetto() != null ? this.soggettoErogazione.getCsASoggetto() : null);
			if (csASoggetto != null) {
				String jsonComuneNascita = getJsonNascitaComuneBean(csASoggetto);

				this.soggettoErogazione.setComuneNascita(jsonComuneNascita);
				this.soggettoErogazione.setNazioneNascita(csASoggetto.getCsAAnagrafica().getStatoNascitaCod());
			}
	...
	private void initDatiResidenza() {
		this.comuneNazioneResidenzaMan = new ComuneNazioneResidenzaMan();

		//Se mi trovo in una nuova erogazione recupero i dati dalla Cartella sociale
		//Se sono in visualizza/modifica trovo valorizzati i dati relativi al MastSogg
		if (StringUtils.isBlank(this.soggettoErogazione.getComuneResidenza()) && StringUtils.isBlank(this.soggettoErogazione.getNazioneResidenza())
				&& StringUtils.isBlank(this.soggettoErogazione.getViaResidenza())) {
			this.sincronizzaResidenza(this.soggettoErogazione);
		}
	...
	public void inizializzaErogazione(Long tipoInterventoId, Long tipoInterCustomId, Long interventoId, SoggettoErogazioneBean soggettoErogazione,
			List<SoggettoErogazioneBean> beneficiari, boolean dentroFascicolo) throws Exception {
		...
		this.loadListaStatiInterventi(tipoInterCustomId != null ? tipoInterCustomId : tipoInterventoId);
		this.buildInterventoErogazzAttList(tipoInterCustomId != null ? tipoInterCustomId : tipoInterventoId);
		//loadListaSettori();
		...
		this.resetBoolDisabledSpese();
		if (this.interEsegHistory.size() > 0) {
			this.nuovoIntEsegMast = this.interEsegHistory.get(0).getCsIInterventoEsegMast();
			this.setBoolDisabledSpese();
			//TODO set organizz erogante, altra org coinvolta, motivo coinvolg
			this.selSettoreEroganteId = this.nuovoIntEsegMast.getSettoreErogante() != null ? this.nuovoIntEsegMast.getSettoreErogante().getId() : null;
			//this.selSettoreTitolareId=nuovoIntEsegMast.getSettoreTitolare()!=null ? nuovoIntEsegMast.getSettoreTitolare().getId() : null;
			this.loadListaOperatoriEroganti(this.selSettoreEroganteId);

			/*	if(nuovoIntEsegMast.getFfOrigineId()!=null)
					this.origineFinanziamentoId= nuovoIntEsegMast.getFfOrigineId();*/

			if (this.nuovoIntEsegMast.getCsOOperatoreSettore().getCsOOperatore().getId() != getCurrentOpSettore().getCsOOperatore().getId())
				this.testataDisabled = true;
		} else {
			// this.codFinanziamento=null;
			this.nuovoIntEsegMast = new CsIInterventoEsegMast();
		...
		if (this.nuovoIntEsegMast.getTipoBeneficiario() == null || this.nuovoIntEsegMast.getTipoBeneficiario().isEmpty()) {
			this.nuovoIntEsegMast.setTipoBeneficiario(DataModelCostanti.ListaBeneficiari.INDIVIDUALE);
			this.nuovoIntEsegMast.setTotBeneficiari(Integer.valueOf(1));
		}
		this.nuovoIntEsegMast.setInterventoProgrammatoId(csIIntervento != null ? csIIntervento.getId() : null);
		...
		if (this.selSettoreEroganteId != null && this.settoreSelezionato != null && this.getListaIdSettoriByIntervervento() != null && this.getListaIdSettoriByIntervervento().size() > 0) {
			this.setVisualizzaPannelloDettaglio(true);
		} else {
			this.setVisualizzaPannelloDettaglio(false);
		}
		//#ROMACAPITALE fine
	...
	/**
	 * 
	 * <h1>inizializzaDaErogazioneMaster</h1>
	 *
	 * <p>
	 * </p>
	 *
	 * @param tipoInterventoId
	 * @param tipoInterCustomId
	 * @param master
	 * @param flgDaFascicolo
	 *
	 * @since 1.26.13
	 * @version 1.0.0
	 * 
	 * @lastUpdate 2025-07-31 - DDV
	 */
	public void inizializzaDaErogazioneMaster(Long tipoInterventoId, Long tipoInterCustomId, CsIInterventoEsegMast master, boolean flgDaFascicolo) {
		...
		this.dentroFascicolo = flgDaFascicolo;
		...
		if (!master.getBeneficiari().isEmpty() && master.getBeneficiari().size() != beneficiari.size()) {
			for (CsIInterventoEsegMastSogg sm : master.getBeneficiari()) {
				if (!sm.getRiferimento()) {
					...
					this.soggettoErogazione = new SoggettoErogazioneBean(sm);
				}
			}
		}
		...
		this.erogazioneHistory = new InterventoErogazHistoryBean();
		this.interEsegHistory = this.erogazioneHistory.initialize(this.mappaErogStatoAttr, master);
		this.erogazioniEsportate = this.verificaErogazioniEsportate(this.interEsegHistory);
		...
		if (this.interEsegHistory.size() > 0) {
			...
			//#ROMACAPITALE			
			this.setVisualizzaPannelloDettaglio(false);
			this.settoreSelezionato = super.findSettoreById(this.selSettoreEroganteId);

			if (this.selSettoreEroganteId != null && settoreSelezionato != null && this.getListaIdSettoriByIntervervento() != null && this.getListaIdSettoriByIntervervento().size() > 0) {
				this.setVisualizzaPannelloDettaglio(true);
			} else {
				this.setVisualizzaPannelloDettaglio(false);
			}
		...
		if (this.isErogazioniPresenti())
			this.testataDisabled = !this.isOperatoreProprietario();

		this.testataDisabled = this.testataDisabled || this.isErogazioniPresenti();

		setCsIPs(master.getCsIPs() != null ? master.getCsIPs() : new CsIPs()); //SISO-498
		//SISO_1138
		String sesso = master.getBeneficiarioRiferimento().getSesso();
		this.sessoBeneficiario = new SessoBean(sesso);
		//SISO-1138 FINE
		...
		this.verificaAllineamentoSoggettoCaso();
		
		this.tipoSoggettoBeneficiario = master.getTipoSoggettoBeneficiario() == null ? "" : master.getTipoSoggettoBeneficiario();
		if (!this.isSempliceBeneficiario() && master.getIdTipoSoggettoBeneficiario() != null) {
			this.inputRagioneSociale = master.getIdTipoSoggettoBeneficiario().getRagioneSociale();
			this.inputCodiceFiscale = master.getIdTipoSoggettoBeneficiario().getCodiceFiscale();
		}
	...
	private void verificaAllineamentoSoggettoCaso() {
		...
		if (csASoggetto != null) {
			...
			this.soggettoErogazione.verificaAllineamentoCaso(viaResidenzaCaso, comuneResidenzaCaso, statoResidenzaCaso, comuneNascitaCaso);
		}
	}
	...
	private void initRisorsaFamiliareBean() {
		...
		if (this.isNucleoBeneficiario() && this.dentroFascicolo)
			this.risorsaFamBean = new RisorsaFamiliareBean(this.soggettoErogazione.getCf());
	}
	...
	public void initDatiSituazioneEconomicaVerificata(Long idTipoIntervento) {
		...
		this.loadDatiIsee(); // MOD-RL
		this.loadlListaCodiciPrestazione(idTipoIntervento);// MOD-RL

	}

	private void resetBoolDisabledSpese() {
		this.testaPercOrValue = DataModelCostanti.ErogazioneInterventoConst.TESTATA_SPESA_PERCENTUALE;
		//		this.calcolaCompartDaDettaglio = true;
		this.calcolaSpesaDaDettaglio = true;

		this.isSpesaPercTesta = true;
		this.isSpesaPercDetta = true;
		this.isSpesaValTesta = false;
		this.isSpesaValDetta = false;
	...
	private void setBoolDisabledSpese() {
		...
		if (nuovoIntEsegMast.getFlagSpesaCalc() == null) {
			boolean totaleVal = (nuovoIntEsegMast.getSpesa() != null && nuovoIntEsegMast.getSpesa().compareTo(new BigDecimal(0)) != 0)
		...
		if (calcolaSpesaDaDettaglio) {
			...
			this.testaPercOrValue = (nuovoIntEsegMast.getPercGestitaEnte() != null && nuovoIntEsegMast.getPercGestitaEnte().compareTo(new BigDecimal(0)) != 0) ? "testaPercentuale"
					: "testaValore";
	...
	public boolean validaTestata() {
		...
		if (this.erogazionePossibile) {
			...
			if (!this.soggettoErogazione.isValorizzato()) {
				addWarning("Beneficiari", "Anagrafica incompleta del soggetto beneficiario di riferimento");
				valido = false;
			} else if (!this.validaCittadinanza(this.soggettoErogazione.getCittadinanza())) {
				addWarning("Beneficiari", "La cittadinanza del soggetto beneficiario di riferimento non è più valida");
				valido = false;
			} else if (!StringUtils.isEmpty(this.soggettoErogazione.getCittadinanza2()) && !this.validaCittadinanza(this.soggettoErogazione.getCittadinanza2())) {
				addWarning("Beneficiari", "La seconda cittadinanza del soggetto beneficiario di riferimento non è più valida");
				valido = false;
			} else
				lstcf.add(this.soggettoErogazione.getCf().toUpperCase());

			if (!this.isUnicoBeneficiario()) {

				if (this.nuovoIntEsegMast.getTotBeneficiari() == null || this.nuovoIntEsegMast.getTotBeneficiari().intValue() == 0) {
				...
				if (!this.getAltriSoggetti().isEmpty()) {
					...
					if (countAnagrafiche.compareTo(this.nuovoIntEsegMast.getTotBeneficiari()) > 0) {
			...
			if (this.isGruppoBeneficiario()) {
				
				if (StringUtils.isBlank(this.tipoSoggettoBeneficiario)) {
					this.addErrorCampiObbligatori("Beneficiari", "Il campo 'Tipo Soggetto' è obbligatorio");
					valido = false;
				}
				
				if (!this.isSempliceBeneficiario()) {
					
					if (this.selectedTipoSgtBeneficiario == null || StringUtils.isBlank(this.selectedTipoSgtBeneficiario.getRagioneSociale())) {
						this.addErrorCampiObbligatori("Beneficiari", "Il campo 'Ragione sociale ente / associazione' è obbligatorio");
						valido = false;
					}
			...
			//			}
			if (this.selSettoreEroganteId == null || this.selSettoreEroganteId.equals(0)) {
				valido = false;
				this.addErrorCampiObbligatori("Erogazioni", "Settore erogante");
			}

			// INIZIO SISO-556
			if (this.nuovoIntEsegMast.getValoreGestitaEnte() != null && this.nuovoIntEsegMast.getSpesa() != null
					&& this.nuovoIntEsegMast.getValoreGestitaEnte().compareTo(this.nuovoIntEsegMast.getSpesa()) > 0) {
				addWarning("Erogazioni",
						"Valore quota a carico dell'ente di testata non corretto:inserire valore numerico intero o decimale (es.12,34), minore o uguale alla Spesa");
				valido = false;
			}
			//FINE SISO-556
			BigDecimal totaleCompartecipazioni = BigDecimal.ZERO;

			if (this.validaImporto(this.nuovoIntEsegMast.getCompartSsn(), "Compartecipazione SSN alla Spesa"))
				totaleCompartecipazioni = totaleCompartecipazioni.add(this.nuovoIntEsegMast.getCompartSsn());
			if (this.validaImporto(this.nuovoIntEsegMast.getCompartUtenti(), "Compartecipazione Utenti alla Spesa"))
				totaleCompartecipazioni = totaleCompartecipazioni.add(this.nuovoIntEsegMast.getCompartUtenti());
			if (this.validaImporto(this.nuovoIntEsegMast.getCompartAltre(), "Compartecipazione SSN alla Spesa"))
				totaleCompartecipazioni = totaleCompartecipazioni.add(this.nuovoIntEsegMast.getCompartAltre());
			if (totaleCompartecipazioni.compareTo(this.nuovoIntEsegMast.getSpesa()) > 0) {
			...
			if (quotaEnte != null) {
				...
				if (!quote.equals(this.nuovoIntEsegMast.getSpesa().setScale(2, RoundingMode.HALF_EVEN))) {
					addWarning("Erogazioni", "La somma delle compartecipazioni (" + totaleCompartecipazioni + " €) e della quota a carico dell'ente (" + quotaEnte
							+ " €) deve coincidere con il totale della spesa");
					valido = false;
				}
			} else {
				//Calcola
				...
				BigDecimal percCalc = this.nuovoIntEsegMast.getSpesa().doubleValue() > 0
						? quotaEnte.multiply(new BigDecimal(100)).divide(this.nuovoIntEsegMast.getSpesa()) : BigDecimal.ZERO;
				
				this.nuovoIntEsegMast.setPercGestitaEnte(percCalc.setScale(2, RoundingMode.HALF_EVEN));
			}

			if (!this.validaDatiCsIPs())
				valido = false;
			
			if (this.esisteInterventoDuplicato())
				valido = false;
	...
	public boolean salva(CsIIntervento intervento, CsIQuota quota, boolean dentroCaso, CsASoggettoLAZY csASoggetto, String frequenzaAccessoAlServizio, CsIInterventoPr progetto) {
		...
		this.nuovoIntEsegMast.setCsIQuota(quota);
		this.nuovoIntEsegMast.setCsIInterventoPr(progetto);

		if (this.isGruppoBeneficiario()) {
			
			this.nuovoIntEsegMast.setTipoSoggettoBeneficiario(this.tipoSoggettoBeneficiario);
		
			if (!this.isSempliceBeneficiario()) {
				
				BaseDTO dtoCsTbTipoSgtBeneficiario = new BaseDTO();
				fillEnte(dtoCsTbTipoSgtBeneficiario);
				
				CsTbTipoSgtBeneficiario csTbTipoSgtBeneficiario = new CsTbTipoSgtBeneficiario();
	//			csTbTipoSgtBeneficiario.setAbilitato(this.selectedTipoSgtBeneficiarioRagioneSociale.isAbilitato());
	//			csTbTipoSgtBeneficiario.setDtIns(this.selectedTipoSgtBeneficiarioRagioneSociale.getDtIns());
	//			csTbTipoSgtBeneficiario.setId(this.selectedTipoSgtBeneficiarioRagioneSociale.getId());
	//			csTbTipoSgtBeneficiario.setUserIns(this.selectedTipoSgtBeneficiarioRagioneSociale.getUserIns());
				csTbTipoSgtBeneficiario.setRagioneSociale(this.selectedTipoSgtBeneficiario.getRagioneSociale());
				csTbTipoSgtBeneficiario.setCodiceFiscale(this.selectedTipoSgtBeneficiario.getCodiceFiscale());
				
				dtoCsTbTipoSgtBeneficiario.setObj(csTbTipoSgtBeneficiario);
				
				csTbTipoSgtBeneficiario = this.interventoService.salvaGruppoBeneficiario(dtoCsTbTipoSgtBeneficiario);
				
				this.nuovoIntEsegMast.setIdTipoSoggettoBeneficiario(csTbTipoSgtBeneficiario);
			
			} else {
				this.nuovoIntEsegMast.setIdTipoSoggettoBeneficiario(null);
			}
		...
		try {
			...
			this.valorizzaInterventoEsegMast(intervento);

			// frida
			if (this.erogazioneHistory.getRows() != null) {
				if (this.erogazioneHistory.getRows().size() > 0) {
					for (InterventoErogazHistoryRowBean row : this.erogazioneHistory.getRows()) {

						dto.setObj(this.nuovoIntEsegMast);
						this.nuovoIntEsegMast = this.interventoService.salvaInterventoEseguitoMast(dto);

						if (row.isNuovaErogazione()) {
							CsIInterventoEseg nuovoIntEseg = valorizzaInterventoEseg(row);
							nuovoIntEseg = this.nuovoIntEsegMast.addCsIInterventoEseg(nuovoIntEseg);
							this.valorizzaIntEsegValori(row, nuovoIntEseg);
							...
							if (row.getIntEseg().getStato().getTipo().equalsIgnoreCase(DataModelCostanti.TipoStatoErogazione.EROGATIVO)) {
								...
								*/
								...
								CsIInterventoPr csIInterventoPr = this.nuovoIntEsegMast.getCsIInterventoPr();
			...
			for (InterventoErogazHistoryRowBean eliminaInetEseg : this.erogazioneHistory.getDeleteErogaz()) {
			...
			this.nuovoIntEsegMast.setBeneficiari(null);

			//Valorizzo dati del soggetto beneficiario principale
			//SISO-760

			this.soggettoErogazione.setRiferimento(true);
			this.valorizzaResidenza();
			this.valorizzaLuogoDiNascita();
			this.valorizzaSesso();

			CsIInterventoEsegMastSogg brif = this.valorizzaSoggettiErogazione(this.soggettoErogazione);
			brif = this.nuovoIntEsegMast.addBeneficiario(brif);
			...
			for (SoggettoErogazioneBean se : this.getAltriSoggetti()) {
				...
				b = this.nuovoIntEsegMast.addBeneficiario(b);
			}

			/* Registrazione su DB */
			if (this.nuovoIntEsegMast.getCsIInterventoEsegs() != null && !this.nuovoIntEsegMast.getCsIInterventoEsegs().isEmpty() && this.nuovoIntEsegMast.getBeneficiari() != null
					&& !this.nuovoIntEsegMast.getBeneficiari().isEmpty()) {

				String codiceFiscale = this.soggettoErogazione.getCf();
				String benefLuogoNascita = codiceFiscale.substring(11, 15);
				//SISO-1138
				if (this.sessoBeneficiario != null)
					this.csIPs.setBenefSesso(this.sessoBeneficiario.getSesso().equalsIgnoreCase("F") ? 2 : 1);
				else
					logger.error("Sesso beneficiario è nullo " + codiceFiscale);
				//SISO-1138 FINE
				this.csIPs.setBenefLuogoNascita(benefLuogoNascita);

				Integer iso3166 = getIso3166ByNazionalita(this.soggettoErogazione.getCittadinanza());
				this.csIPs.setBenefCittadinanza(iso3166);

				Integer iso3166_2 = getIso3166ByNazionalita(this.soggettoErogazione.getCittadinanza2());
				this.csIPs.setBenefCittadinanza2(iso3166_2);

				//SISO-962 inizio
				// L'indirizzo deve essere pescato dalla MastSogg
				String benefNazione = null;
				if (this.comuneNazioneResidenzaMan.isComune()) {
					String codComIstat = this.comuneNazioneResidenzaMan.getComuneMan().getComune().getCodIstatComune();
					...
					if (resInItalia) {
						...
						if (regione != null && regione.getCodAgenziaEntrate() != null)
							...
							if ("BZ".equalsIgnoreCase(comune.getSiglaProv()))
								codAE = "03";
							if ("TN".equalsIgnoreCase(comune.getSiglaProv()))
								codAE = "18";
						}

						this.csIPs.setBenefRegione(codAE);
						benefNazione = DataModelCostanti.AmTabNazioni.CODICE_ISO3166_ITALIA;
						//csIPs.setBenefNazione(benefNazione);
					}
				} else if (this.comuneNazioneResidenzaMan.isNazione())

					if (this.comuneNazioneResidenzaMan.getNazioneMan().getNazione() == null && this.soggettoErogazione.isNazioneResidenzaNonDefinita()) {
						benefNazione = null;
					} else {
						benefNazione = this.comuneNazioneResidenzaMan.getNazioneMan().getNazione().getIso3166();
					}

				benefNazione = benefNazione != null ? benefNazione : DataModelCostanti.AmTabNazioni.CODICE_ISO3166_NON_DISPONIBILE;
				this.csIPs.setBenefNazione(benefNazione);
				//SISO-962 Fine

				this.csIPs.setDenomPrestazione(findDenominazionePrestazione(this.csIPs.getCodPrestazione()));

				if (frequenzaAccessoAlServizio.equals(TIPO_FREQUENZA_SERVIZIO.UNATANTUM.getCodice()))
					this.csIPs.setCarattere(DataModelCostanti.CSIPs.CARATTERE_PRESTAZIONE_DI_TIPO_OCCASIONALE);
				else //frequenza PERIODICO E IRREGOLARE
					this.csIPs.setCarattere(DataModelCostanti.CSIPs.CARATTERE_PRESTAZIONE_DI_TIPO_PERIODICO);

				this.protDsuMan.valorizza(this.csIPs);

				this.nuovoIntEsegMast.setCsIPs(this.csIPs);
				csIPs.setCsIInterventoEsegMast(this.nuovoIntEsegMast);
				// FINE MOD-RL
			...
			if (!storicoDaCancellare.isEmpty()) {
				...
				this.interventoService.eliminaInterventoEsegStorico(dto);
			}
			...
			if (this.isUnicaErogazioneChiusura()) {
				for (InterventoErogazHistoryRowBean interventoErogazHistoryRowBean : historyRowBeanList) {
					if (interventoErogazHistoryRowBean.getStato().getFlagChiudi().equals(DataModelCostanti.CsCfgIntEsegStato.FLAG_CHIUDI_CHIUSO)) {
						addWarning("Erogazioni",
								"Stato " + interventoErogazHistoryRowBean.getStato().getNome() + " non consentito per prestazione "
										+ TIPO_FREQUENZA_SERVIZIO.IRREGOLARE.getDescrizione() + ", modificare eventualmente la prestazione in "
	...
	private void loadListaOperatoriEroganti(Long settoreId) {
		try {
			this.listaOperAnagrafica = new ArrayList<SelectItem>();
			...
			if (settoreId != null) {
				...
				this.listaOperAnagrafica.addAll(convertiLista(lstOpSettAnagrafica));
			}
	 ...
	 */
	public static boolean controllaGiorni(Date dataErog, Date dataErogA, Date dataErog2, Date dataErogA2) {
	...
	public void aggiungiAttributi(boolean rendicontoPeriodico, String flagPeriodoRipartiz, boolean visualizzaIntervalloDateErogazione, String unitaDiMisura) { //SISO-556 SISO-958
		...
		//Clear
		for (InterventoErogazAttrBean attr : this.mappaErogStatoAttr.get(stato.getId()))
			attr.reset2default();
		this.resetOrganizAndNuovoIntEseg();
		...
		if (nuovocsIntEseg.getValoreGestitaEnte() != null) {
			if (nuovocsIntEseg.getSpesa() == null) {
				...
				addMessage(FacesMessage.SEVERITY_ERROR,
						"Valore quota a carico dell'ente in dettaglio non corretto:inserire valore numerico intero o decimale (es.12,34), minore o uguale alla Spesa");
			}
		}
	...
	public void setTestaPercValue(AjaxBehaviorEvent event) {
		...
		if (this.testaPercOrValue.equals(DataModelCostanti.ErogazioneInterventoConst.TESTATA_SPESA_PERCENTUALE)) {
			this.isSpesaValTesta = false;
			if (!this.calcolaSpesaDaDettaglio) { //se la spesa è già stata settata nel dettaglio vuol dire che nella testata c'è la somma dei dettagli e non deve essere cancellata
				this.nuovoIntEsegMast.setValoreGestitaEnte(null);
				this.isSpesaPercTesta = true;
			}
		} else {
			this.isSpesaPercTesta = false;
			if (!this.calcolaSpesaDaDettaglio) {
				this.nuovoIntEsegMast.setPercGestitaEnte(null);
				this.isSpesaValTesta = true;
			}
		}
	}
	...
	public List<SelectItem> getLstSelItemStatiErog(String frequenzaAccessoAlServizio) {
		if (listaStatiErogazione != null) {
			...
			for (CsCfgIntEsegStato stato : listaStatiErogazione) {
				...
				if (frequenzaAccessoAlServizio != null && frequenzaAccessoAlServizio.equals(TIPO_FREQUENZA_SERVIZIO.UNATANTUM.getCodice())
						&& TipoStatoErogazione.EROGATIVO.equals(stato.getTipo()) && DataModelCostanti.CsCfgIntEsegStato.FLAG_CHIUDI_APERTO == stato.getFlagChiudi()) {
					rimuovoStato = true;
				}
				...
				if (!rimuovoStato) {
					...
					case 100:
						...
						if (g1 == null)
							g1 = new SelectItemGroup(stato.getDescFase());
						break;
					case 200:
						lst2.add(new SelectItem(stato.getId(), stato.getNome()));
						if (g2 == null)
							g2 = new SelectItemGroup(stato.getDescFase());
						break;
					case 300:
						lst3.add(new SelectItem(stato.getId(), stato.getNome()));
						if (g3 == null)
							g3 = new SelectItemGroup(stato.getDescFase());
						break;
					case 400:
						lst4.add(new SelectItem(stato.getId(), stato.getNome()));
						if (g4 == null)
							g4 = new SelectItemGroup(stato.getDescFase());
						break;
					}
				}
				//FINE SISO-556
	 ...
	 */
	public boolean isSempliceBeneficiario() {
		return this.tipoSoggettoBeneficiario == "" || this.tipoSoggettoBeneficiario.equalsIgnoreCase("Semplice beneficiario");
	}

	public List<String> loadListaGruppiBeneficiariRagioneSociale(String query) {
		List<CsTbTipoSgtBeneficiario> resultQuery = new ArrayList<CsTbTipoSgtBeneficiario>();
		...
		if (!query.isEmpty()) {
			baseDTO.setObj(query);

			resultQuery = this.interventoService.findGruppoBeneficiarioPerRagioneSociale(query);

			listReturn = this.objectToString(resultQuery);
			
			this.disableInputCodiceFiscale = false;
		} else {
			this.disableInputCodiceFiscale = true;
		}
	...
	/**
	 * 
	 * <h1>loadListaGruppiBeneficiariCodiceFiscale</h1>
	 *
	 * <p>
	 * </p>
	 *
	 ...
	 * @lastUpdate 2025-07-24 - DDV
	 */
	public List<String> loadListaGruppiBeneficiariCodiceFiscale(String query) {
		List<CsTbTipoSgtBeneficiario> resultQuery = new ArrayList<CsTbTipoSgtBeneficiario>();
		...
		if (!query.isEmpty()) {
			...
			resultQuery = this.interventoService.findGruppoBeneficiarioPerCodiceFiscale(this.selectedTipoSgtBeneficiario.getRagioneSociale(), query);
	 ...
	 * 
	 * @author DDV
	 * @lastUpdate 2025-08-04 - DDV
	 */
	private List<String> objectToString(List<?> listToCast) {
		
		List<String> listStringReturn = new ArrayList<String>(listToCast.size());
		
		for (Object object : listToCast) {
			listStringReturn.add(object != null ? object.toString() : null);
		}
	 ...
	 * @lastUpdate 2025-08-04 - DDV
	 */
	public void onGruppiBeneficiari(javax.faces.event.AjaxBehaviorEvent event) {
		this.selectedTipoSgtBeneficiario = new CsTbTipoSgtBeneficiario();
		
		this.selectedTipoSgtBeneficiario.setRagioneSociale(this.inputRagioneSociale);
		this.selectedTipoSgtBeneficiario.setCodiceFiscale(this.inputCodiceFiscale);
		this.disableInputCodiceFiscale = false;
	}
	...
	public void addSoggettoFamiliare() {
		...
		if (r != null && r.getSoggettoCaso() != null) {
			...
			String jsonComuneNascita = getJsonNascitaComuneBean(s);
	...
	public void addDaAnagrafeWrapper(DatiUserSearchBean s) {
		...
		if (s != null) {
			...
			if (anaSearch) {
				...
				sero = new SoggettoErogazioneBean(p.getCognome(), p.getNome(), p.getCodfisc(), p.getCittadinanza(), p.getDataNascita(), p.getIndirizzoCivicoResidenza(), json,
						nazioneResidenza, jsonN, nazioneNascita, p.getSesso(), false);
	...
	public List<String> getListaCittadinanze() {
		if (listaCittadinanze == null) {
			...
			try {
				AccessTableNazioniSessionBeanRemote bean = (AccessTableNazioniSessionBeanRemote) ClientUtility.getEjbInterface("CarSocialeA", "CarSocialeA_EJB",
	...
	private boolean checkProtInps(ErogazionePrestazioneDTO intervento) {
		...
		if (!StringUtils.isBlank(intervento.getPrefixProtDsu()) && !StringUtils.isBlank(intervento.getNumProtDsu()) && !StringUtils.isBlank(intervento.getProgProtDsu())) {
			if (numProtocollo != null && numProtocollo.equals(intervento.getNumProtDsu()) && progProtDsu.equals(intervento.getProgProtDsu())
	...
	public String getDataEventoLabel() {
		if (csIPs != null && "A1.05.01".equals(csIPs.getCodPrestazione())) //SISO-2384
			...
			this.dataEventoLabel = DATA_EVENTO_LABEL_DEFAULT;
