...
public class SalvaUtente extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if ("resetPwd".equals(mode)) {				
				BaseAction.resetPwdUtente(request);
				
				RequestDispatcher dispa = request.getRequestDispatcher("/jsp/newUser.jsp");
				dispa.forward(request, response);
				return;
			}
	...
	private boolean validate(HttpServletRequest request) {
		...
		String mode = request.getParameter("mode");
		boolean ctrlOldPwd = mode == null || !mode.equalsIgnoreCase("new");
		...
		if (ctrlOldPwd && oldpassword.length() == 0) {
			request.setAttribute("uN", userName);
		...
		if (doCtrls) {
			...
			if (livSicurCredenz.toUpperCase().equals("C")) {
				String ctrlError = doCLevelCtrls(oldpassword, password, ctrlOldPwd);
	...
	private String doCLevelCtrls(String oldpassword, String password, boolean ctrlOldPwd) {		
	...
	private void setAnagraficaAttribute(HttpServletRequest request, AmAnagrafica ana) {
		...
		try {
			...
			request.setAttribute("dtInsAna", ana.getDtIns()!=null ? sdf.format(ana.getDtIns()) : null);
			...
			request.setAttribute("dtNascita", ana.getDataNascita()!=null ?  sdf.format(ana.getDataNascita()) : null);
