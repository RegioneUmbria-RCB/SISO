...
@Stateless
public class ComuneServiceBean extends CommonServiceBean implements ComuneService {
	...
	public List<KeyValueDTO> getListaComuneByUsername(String username, boolean visMissingParam) {
		...
		try {
			...
			Query q = manager.createNativeQuery(ComuneQueryBuilder.SQL_LISTA_COMUNI_BY_USER);
	...
	public List<KeyValueDTO> getListaInstanceByComune(String comune,
		...
		try {
			...
			Query q = manager.createNativeQuery(ComuneQueryBuilder.SQL_LISTA_INSTANCE_COMUNI);
	...
	public List<KeyValueDTO> getListaFonteByComune(String comune, boolean visMissingParam) {
		...
		try {
			...
			Query q = manager.createNativeQuery(ComuneQueryBuilder.SQL_LISTA_FONTE_COMUNI);
			q.setParameter("comune", comune);
			List<Object[]> result = q.getResultList();
			for (Object[] rs : result) {

				String label = rs[2].equals('S') ? (String) rs[1] + (visMissingParam ? "*" : "") : (String) rs[1];
				KeyValueDTO item = new KeyValueDTO(((BigDecimal) rs[0]).toString(), label);
				listSelect.add(item);
