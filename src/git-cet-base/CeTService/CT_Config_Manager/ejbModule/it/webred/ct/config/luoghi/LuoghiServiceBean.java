...
public class LuoghiServiceBean implements LuoghiService {
	...
	public AmTabComuni getComuneItaByIstat(String codIstat){
		...
		if(codIstat != null && !codIstat.trim().isEmpty()){
	...
	public List<AmTabNazioni> getNazioni(){
		...
		try {
			Query q = manager.createNamedQuery("AmTabNazioni.getNazioni");
			return q.getResultList();
	...
	//INIZIO MOD-RL 
	public AmTabNazioni getNazioneByNazionalita(String nazionalita){

		try {
			Query q = manager.createNamedQuery("AmTabNazioni.getNazioneByNazionalita");
			q.setParameter("nazionalita", nazionalita);
			List<AmTabNazioni> lista = q.getResultList();
			if(lista.size() > 0)
				return lista.get(0);
			
			logger.warn("LuoghiServiceBean.getNazioneByNazionalita: " + nazionalita + " - No Results");
			return null;
	...
	public AmTabNazioni getNazioneByCodiceGenerico(String codice) {
		...
		AmTabNazioni nazione = this.getNazioneByIstat(codice);
