...
public class CommonServiceBean{
	...
	public List<AmKeyValueDTO> getListaKeyValue(ParameterSearchCriteria criteria) {
		...
		try {
			...
			logger.debug("getListaKeyValue - SQL[" + sql + "]");
			Query q = manager.createNativeQuery(sql);

			List<Object[]> result = q.getResultList();
			logger.debug("getListaKeyValue - size[" + result.size() + "]");
			for (Object[] rs : result) {
				...
				ext.setFkAmFonte(fkFonte != null ? new Integer(fkFonte.intValue()) : null);
				...
				try {
					...
					Method getListaAssistedValue = cls.getMethod("getListaAssistedValue", new Class[] {EntityManager.class, String.class });
					Object arglist[] = new Object[2];
					arglist[0] = manager;
					arglist[1] = ext.getValueConf();

					try {
						dto.setListaAssistedValue((List<AssistedValueDTO>) getListaAssistedValue.invoke(cls.newInstance(), arglist));
	...
	public List<AmKeyValue> getListaToOverwrite(String application) {
		...
		Query q = manager.createNamedQuery("AmKeyValue.getAmKeyValueToOverwrite");
		q.setParameter("application", application);
		return (List<AmKeyValue>) q.getResultList();
	...
	public AmKeyValueExt saveParam(AmKeyValueDTO dto) {
		...
		try {
			...
			if (ext.getId() != null) {
				...
				Query q = manager.createNamedQuery("AmKeyValueExt.getAmKeyValueExtById");
	...
	public AmSection getAmSection(String name, String application){
		...
		try {
			...
			sql = new ParameterQueryBuilder().createQueryAmSection(name, application);
			Query q = manager.createQuery(sql);
			return (AmSection) q.getSingleResult();
			
		} catch (Throwable t) {
			throw new ParameterServiceException(sql, t);
		}
	...
	public AmKeyValueExt getAmKeyValueExt(ParameterSearchCriteria criteria) {
		...
		try {
			ParameterQueryBuilder b = new ParameterQueryBuilder(criteria);
			String sql = b.createQueryAmKeyValueExt();
			Query q = manager.createQuery(sql);
			List<AmKeyValueExt> lista = q.getResultList();
			if(lista.size() > 0)
				return lista.get(0);
	...
	public List<AmKeyValueExt> getAmKeyValueExtLike(ParameterSearchCriteria criteria) {

		try {
			String sql = new ParameterQueryBuilder(criteria).createQueryAmKeyValueExtLike();
			Query q = manager.createQuery(sql);
			return (List<AmKeyValueExt>) q.getResultList();
		} catch (Throwable t) {
			throw new ParameterServiceException(t);
		}
	...
	public String getVersioneSmartWelfare(){
		...
		criteria.setKey("versione.smartwelfare");
		AmKeyValueExt amKey = this.getAmKeyValueExt(criteria);
		versione = amKey.getValueConf()!= null ? amKey.getValueConf() : "";
		return versione;
	}
	...
	public String getVersionePiattaforma(){
		...
		ParameterSearchCriteria criteria = new ParameterSearchCriteria();
		criteria.setKey("versione.piattaforma");
		AmKeyValueExt amKey = this.getAmKeyValueExt(criteria);
		versione = amKey.getValueConf()!= null ? amKey.getValueConf() : "";
		return versione;
	}
	
	public HashMap<String, String> getListaParametriGlobali(String descrizione) {
		HashMap<String, String> mappa = new HashMap <String, String>();
		ParameterSearchCriteria criteria = new ParameterSearchCriteria();
		criteria.setKey(descrizione);
		criteria.setSection("param.globali");
		List<AmKeyValueExt> amKeys = this.getAmKeyValueExtLike(criteria);
		for(AmKeyValueExt amKey : amKeys){
			if (amKey != null && amKey.getValueConf() != null && !amKey.getValueConf().trim().isEmpty())
				mappa.put(amKey.getKeyConf(), amKey.getValueConf());
		}
		return mappa;
	}
