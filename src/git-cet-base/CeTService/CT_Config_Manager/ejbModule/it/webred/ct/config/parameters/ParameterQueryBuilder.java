...
public class ParameterQueryBuilder extends AbstractQueryBuilder{
	...
	private final String SQL_LISTA_KEY_VALUE_EXT = "select kve from AmKeyValueExt kve left join fetch kve.amSection s where kve.id is not null ";

	private final String SQL_GET_SECTION = "select section from AmSection section " + "where section.name = '@NAME' ";
	...
	public String createQueryAmKeyValue() {
		...
		sql += " order by kv.SECTION_NAME, kv.KEY_CONF";
	...
	public String createQueryAmKeyValueExt() {
		...
		sql += " order by kve.keyConf";

		return sql;

	}
	
	public String createQueryAmKeyValueExtLike() {

		String sql = SQL_LISTA_KEY_VALUE_EXT;
		String whereCond = getSQLCriteriaLike();
	...
	private String getSQLCriteriaLike() {
		String sqlCriteria = "";

		sqlCriteria = (key!=null && !key.trim().isEmpty() ? addOperator(sqlCriteria) + " kve.keyConf like '" + key + "%'" : "");
		
		sqlCriteria = (section!=null && !section.trim().isEmpty() ? addOperator(sqlCriteria) + " kve.sectionName = '" + section + "'" : "");
