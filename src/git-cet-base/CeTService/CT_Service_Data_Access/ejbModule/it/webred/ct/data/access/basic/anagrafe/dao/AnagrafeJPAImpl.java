...
public class AnagrafeJPAImpl extends CTServiceBaseDAO implements AnagrafeDAO  {
	...
	public List<SitDPersona> getListaPersoneByDatiAnag(RicercaSoggettoAnagrafeDTO criteri) throws AnagrafeException{
		...
		try {
			String sql = "SELECT p FROM SitDPersona p WHERE ";
			
			String sqlCriteria = "p.dtFineVal IS NULL ";
			sqlCriteria = (criteri.getCodFis() == null  || criteri.getCodFis().trim().equals("") ? sqlCriteria : sqlCriteria + " AND UPPER(p.codfisc) ='" + criteri.getCodFis().trim().toUpperCase() + "'");
			sqlCriteria = (criteri.getCognome() == null  || criteri.getCognome().trim().equals("") ? sqlCriteria : sqlCriteria + " AND UPPER(p.cognome) like '%" + criteri.getCognome().trim().replace("'","''").toUpperCase() + "%'");
			sqlCriteria = (criteri.getNome() == null  || criteri.getNome().trim().equals("") ? sqlCriteria : sqlCriteria + " AND UPPER(p.nome) like '%" + criteri.getNome().trim().replace("'","''").toUpperCase() + "%'");
			sqlCriteria = (criteri.getSesso() == null  || criteri.getSesso().equals("") ? sqlCriteria : sqlCriteria + " AND UPPER(p.sesso)='" + criteri.getSesso().toUpperCase() + "'");
			sqlCriteria = (criteri.getIdSogg() == null  || criteri.getIdSogg().equals("")) ? sqlCriteria : sqlCriteria + " AND p.idExt = '"+criteri.getIdSogg()+"'";		
			sqlCriteria = (criteri.getIdVarSogg() == null  || criteri.getIdVarSogg().equals("")) ? sqlCriteria : sqlCriteria + " AND p.id = '"+criteri.getIdVarSogg()+"'";		
			
			String dataStr="";
			SimpleDateFormat df = new SimpleDateFormat("dd/MM/yyyy");
			if (criteri.getDtNas()!= null) {
				dataStr = df.format(criteri.getDtNas());
				sqlCriteria = sqlCriteria + " AND p.dataNascita =TO_DATE('" + dataStr + "','dd/MM/yyyy')";
			}else{
				if(criteri.getAnnoNascitaDa()!=null && criteri.getAnnoNascitaDa()>0){
					sqlCriteria = sqlCriteria + " AND p.dataNascita >= TO_DATE('" + "01/01/"+criteri.getAnnoNascitaDa() + "','dd/MM/yyyy')";
				}if(criteri.getAnnoNascitaA()!=null && criteri.getAnnoNascitaA()>0){
					sqlCriteria = sqlCriteria + " AND p.dataNascita <= TO_DATE('" + "31/12/"+criteri.getAnnoNascitaA() + "','dd/MM/yyyy')";
				}
			}
			
			sql+= sqlCriteria + " ORDER BY p.cognome, p.nome, p.dataNascita,p.idExtComuneNascita";
			
			logger.debug("searchSoggetto. SQL: " + sql);
			Query q = manager_diogene.createQuery(sql);
			if(criteri.getMaxResult() != null)
				q.setMaxResults(criteri.getMaxResult());
			lista =(List<SitDPersona>) q.getResultList();
		...
		if(param!=null){
			try {
				...
				for (Object[] ele : result) {
					...
					SitDPersona p = (SitDPersona)ele[0];
					if (p!= null){
						comp.setPersona(p);
						comp.setCittadinanza(findCittadinanzaByIdExtStato(p.getIdExtStato()));
					}
	...
	public List<ComponenteFamigliaDTO> getCompFamigliaByCodFis(RicercaSoggettoAnagrafeDTO rs) throws AnagrafeException {
		...
		if(param!=null){
			try {
				...
				q.setParameter("dtRif", dtRif);
				List<Object[]> result = q.getResultList();
				logger.warn("getCompFamigliaByCodFis [CODFISC: " + rs.getCodFis() + "][dt_rif: " + rs.getDtRif()+"] - TROVATO. N.ELE: "+ result.size());
				for (Object[] ele : result) {
					ComponenteFamigliaDTO comp= new ComponenteFamigliaDTO(); 
					SitDPersona p = (SitDPersona)ele[0];
					if (p!= null){
	...
	public String findCittadinanzaByIdExtStato(String idExtStato){
		SitStato s = this.getStato(idExtStato);
		String cittadinanza=null;
		if(s!=null){
			String codice = s.getIdOrig();
			if("100".equals(codice) || "I".equalsIgnoreCase(codice))
				cittadinanza = "ITALIANA";
			else{
				AmTabNazioni n  = luoghiService.getNazioneByCodiceGenerico(codice);
			    cittadinanza = n!=null ? n.getNazionalita() : null;
			}
		}
		return cittadinanza;
	}
	...
	public List<ComponenteFamigliaDTO> getResidentiAlCivico(RicercaIndirizzoAnagrafeDTO ri) throws AnagrafeException {
		...
		try {
			...
			for (Object[] ele : result) {
				...
				SitDPersona p = (SitDPersona)ele[3];
				if (p!= null){
					comp.setPersona(p);
					comp.setCittadinanza(findCittadinanzaByIdExtStato(p.getIdExtStato()));
				}
	...
	public List<IndirizzoAnagDTO> getIndirizzoResidenzaByCodFisc(RicercaSoggettoAnagrafeDTO rs) {
		List<IndirizzoAnagDTO> lst = new ArrayList<IndirizzoAnagDTO>();
		String codFis = rs.getCodFis();
		if (codFis == null) {
			return null;
		}
		...
		try{
			...
			for(Object[] obj : result){
				...
				IndirizzoAnagDTO indirizzoAna = new IndirizzoAnagDTO();
				int idx = -1;
				...
				indirizzoAna.setDataInizioApp((Date)obj[++idx]);
				lst.add(indirizzoAna);
	...
	public List<IndirizzoAnagDTO> getIndirizzoResidenzaByNomeCiv(RicercaCivicoDTO rc) {
		...
		try{
			...
			for(Object[] obj : result){
				...
				int idx = -1;
				indirizzoAna.setCodiceVia((String)obj[++idx]);
				String ind = (String)obj[++idx];
				indirizzoAna.setIndirizzo(ind == null ? null : ind.trim());
				indirizzoAna.setCivicoNumero((String)obj[++idx]);
				indirizzoAna.setCivicoAltro((String)obj[++idx]);
				indirizzoAna.setProv((String)obj[++idx]);
				indirizzoAna.setComCod((String)obj[++idx]);
	...
	private SitStato getStato(String idExtStato){
		try{
			if(idExtStato!=null){
				Query q= manager_diogene.createNamedQuery("SitStato.findAttivoByIdExt");
				q.setParameter("idExtStato", idExtStato);
				List<SitStato> stati = q.getResultList();
