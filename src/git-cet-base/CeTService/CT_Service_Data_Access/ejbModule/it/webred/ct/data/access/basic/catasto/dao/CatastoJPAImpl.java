...
public class CatastoJPAImpl extends CatastoBaseDAO implements CatastoDAO {
	...
	@Override
	public String[] getLatitudineLongitudineFromCivico(RicercaCivicoDTO rc){
		String[] latLon = {"0","0"};
		/*
		 * Se è valorizzato il parametro civicoPk saltiamo la prima query altrimenti
		 * recuperiamo l'ID dalla tabella dei civici e delle strade
		 */
		String civicoId = null;
		if (rc.getIdCivico() != null && !rc.getIdCivico().trim().equalsIgnoreCase("")){
			civicoId = rc.getIdCivico();
		}else if (rc.getToponimoVia() != null && !rc.getToponimoVia().trim().equalsIgnoreCase("") && 
				rc.getDescrizioneVia() != null && !rc.getDescrizioneVia().trim().equalsIgnoreCase("") && 
						rc.getCivico() != null && !rc.getCivico().trim().equalsIgnoreCase("") ){

			String sql = "select siticivi.pkid_civi " +
					"from siticivi " +
					"left join sitidstr on siticivi.pkid_stra = sitidstr.pkid_stra " + 
					"where " +
					"SITIDSTR.DATA_FINE_VAL = TO_DATE('31/12/9999', 'DD/MM/YYYY') " + 
					"AND SITICIVI.DATA_FINE_VAL = TO_DATE('31/12/9999', 'DD/MM/YYYY') " + 
					"AND SITIDSTR.PREFISSO = ? " + 
					"AND SITIDSTR.NOME LIKE ? "+ 
					"AND SITICIVI.CIVICO = ? " +
					"ORDER BY siticivi.pkid_civi ";
			
			logger.info("getLatitudineLongitudineFromCivico:  - SQL["+sql+"]");
			logger.info("Parametri:  - Sedime["+rc.getToponimoVia()+"] Denominazione["+rc.getDescrizioneVia()+"] Civico["+rc.getCivico()+"] ");
			
			if (sql != null) {
				
				Query q = manager_diogene.createNativeQuery(sql);
				q.setParameter(1, rc.getToponimoVia());
				q.setParameter(2, "%" + rc.getDescrizioneVia() + "%");
				q.setParameter(3, rc.getCivico());
				
				//List<Object[]> lstCiviciId = q.getResultList();
				List<Object> lstCiviciId = q.getResultList();
				if (lstCiviciId != null && lstCiviciId.size()>0){
					BigDecimal seqCivico = (BigDecimal)lstCiviciId.get(0);
					civicoId = seqCivico.toString();
				}
		...
		if (civicoId != null){
			logger.info("CIVICO COINVOLTO ID: ["+ civicoId.toString() +"]");			
			String qry = "SELECT DISTINCT t.y , t.x "
					+ "FROM siticivi, "
					+ "TABLE (sdo_util.getvertices ( siticivi.shape ) ) t WHERE siticivi.PKID_CIVI = " + civicoId.toString();

			logger.info("RECUPERO X,Y NEL SISTEMA DI COORDINATE UTILIZZATO - SQL["+qry+"]");
			
			try{
				if (qry != null) {

					// restituisce x e y nel sistema di coordinate usato
					Query q = manager_diogene.createNativeQuery(qry);
					ArrayList<Object> alYX = (ArrayList<Object>)q.getResultList();
					if (alYX != null && alYX.size()>0){
						Object[] oyx = (Object[])alYX.get(0);
						//if (xy != null && xy.length == 2){
						BigDecimal yd = (BigDecimal)oyx[0];
						BigDecimal xd = (BigDecimal)oyx[1];
						/*
						 * Per conoscere il sistema di coordinate (in gauss con srdi 82087 ?!?! )
						 */
						//Double sysCoordinate = new Double(Coordinates.getCoordSystem(xd,yd));
						String sysCoordinate = Coordinates.getCoordSystem(xd.doubleValue(),yd.doubleValue());
						logger.info("Sistema di Coordinate:" + sysCoordinate);
						
						qry = "select t.y lat, t.x lon from  TABLE (sdo_util.getvertices "
								+ "(sdo_cs.transform ( mdsys.sdo_geometry(2001, '"+sysCoordinate+"', mdsys.SDO_POINT_TYPE ("+xd+", "+yd+", NULL), NULL, NULL) , "
								+ "MDSYS.SDO_DIM_ARRAY(MDSYS.SDO_DIM_ELEMENT('X', 1313328, 2820083, 0.0050),MDSYS.SDO_DIM_ELEMENT('Y', 3930191, 5220322.5, 0.0050)), 8307) "
								+ ") ) t ";

						logger.info("RECUPERO LAT, LON - SQL["+qry+"]");
						q = manager_diogene.createNativeQuery(qry);
						ArrayList<Object> alLatLon = (ArrayList<Object>)q.getResultList();
						if (alLatLon != null && alLatLon.size()>0){
							Object[] oLatLon = (Object[])alLatLon.get(0);
							BigDecimal lat = (BigDecimal)oLatLon[0];
							BigDecimal lon = (BigDecimal)oLatLon[1];
							latLon = new String[]{lat.toString(), lon.toString()};
						}
					}
				}
				...
				e.printStackTrace();
				logger.error("IMPOSSIBILE RECUPERARE L'ID DEL CIVICO COINVOLTO ");
			}

		}else{
			logger.debug("IMPOSSIBILE RECUPERARE L'ID DEL CIVICO COINVOLTO ");
		}
