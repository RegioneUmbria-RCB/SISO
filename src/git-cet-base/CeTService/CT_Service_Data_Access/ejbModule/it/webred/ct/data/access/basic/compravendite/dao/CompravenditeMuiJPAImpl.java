...
public class CompravenditeMuiJPAImpl extends CTServiceBaseDAO implements CompravenditeMuiDAO {
	...
	public int getCountCompravenditeFPS(RicercaOggettoCatDTO rc)		throws CompravenditeMUIException {
		logger.debug("Compravendite - getCountCompravenditeFPS() [SEZIONE: "+rc.getSezione()+"];" + "[FOGLIO: "+rc.getFoglio()+"]; " + "[PARTIC.: "+rc.getParticella()+"];[SUB.: "+rc.getUnimm()+"])");
		List<MuiFabbricatiIdentifica>  lista = new ArrayList<MuiFabbricatiIdentifica>();
		try {
			Query q= null;
			String sezione = rc.getSezione();
			String sub = rc.getUnimm();
			q = manager_diogene.createNamedQuery("Join_FabbrIden_Nota_FabbrInfo.getContaNotaBySezFPS");
			q.setParameter("sub",(sub!=null?sub.trim():null));
			q.setParameter("foglio",rc.getFoglio().trim());
			q.setParameter("particella",rc.getParticella().trim());
	...
	public int getCountCompravenditeTerrenoFP(RicercaOggettoCatDTO rc) throws CompravenditeMUIException {
		logger.debug("Compravendite - getCountCompravenditeTerrenoFP() [FOGLIO: "+rc.getFoglio()+"]; " + "[PARTIC.: "+rc.getParticella()+"])");
		List<MuiTerreniInfo>  lista = new ArrayList<MuiTerreniInfo>();
		try {
			Query q= null;
			q = manager_diogene.createNamedQuery("Join_Nota_FabbrInfo.getCountTerrenoByFP");
			q.setParameter("foglio",rc.getFoglio().trim());
			q.setParameter("particella",rc.getParticella().trim());
			lista = (List<MuiTerreniInfo> ) q.getResultList();
			logger.debug("Result size ["+lista.size()+"]");
