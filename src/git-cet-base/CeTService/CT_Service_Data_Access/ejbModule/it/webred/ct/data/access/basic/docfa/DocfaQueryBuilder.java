...
public class DocfaQueryBuilder extends CTQueryBuilder{
	...
	private final String SQL_DATI_DOCFA_BY_FPS =
			"SELECT U.* " +
			"FROM  DOCFA_UIU U " +
			"WHERE LPAD(U.FOGLIO, 4, '0') = LPAD(?,4, '0') " +   
			  "AND LPAD(U.NUMERO, 5, '0') = LPAD(?, 5, '0')" + 
			  "AND LPAD(NVL(U.SUBALTERNO,''), 4,'0')=LPAD(NVL(?,''),4, '0')";
	...
	public String getSQL_DATI_DOCFA_BY_FPS() {
		return SQL_DATI_DOCFA_BY_FPS;
	}
