...
public class DocfaJPAImpl extends CTServiceBaseDAO implements DocfaDAO { 
	...
	@Override 
	public int getCountDocfaUiu(RicercaOggettoDocfaDTO ro){
		List<Object[]> result= null;
		try {
			String sql = (new DocfaQueryBuilder()).getSQL_DATI_DOCFA_BY_FPS();
			
			Query q = manager_diogene.createNativeQuery(sql);
			q.setParameter(1,ro.getFoglio().trim());
			q.setParameter(2,ro.getParticella().trim());
			q.setParameter(3,ro.getUnimm().trim());
			logger.debug("Docfa - getCountDocfaUiu(). SQL: " + sql);
			logger.debug("PARMS: [FOGLIO: "+ ro.getFoglio()+"];[PARTICELLA: "+ ro.getParticella()+"];[SUB: "+ ro.getUnimm()+"];") ;
	...
	public List<DocfaPlanimetrie> getPlanimetriePerSezFglNum(RicercaOggettoDocfaDTO rs) {
		...
		try {
			...
			Query q = manager_diogene.createNamedQuery("DocfaPlanimetrie.getPlanimetriePerSezFglNumMaxProg");
