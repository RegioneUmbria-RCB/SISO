...
public class OmniadocTipoRecordFiles<T extends OmniadocTipoRecordEnv<Testata>> extends ImportFilesWithTipoRecord<T> {
	
	public OmniadocTipoRecordFiles(T env) {
		super(env);
	}//-------------------------------------------------------------------------
	...
	public void preProcesing(Connection con) throws RulEngineException {
		...
		try {
			st = con.createStatement();
			st.execute(env.RE_OMNIADOC_IDX);
		} catch (SQLException e1) {
