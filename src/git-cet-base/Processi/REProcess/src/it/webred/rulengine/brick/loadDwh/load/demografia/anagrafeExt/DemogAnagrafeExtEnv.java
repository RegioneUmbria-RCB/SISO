...
public class DemogAnagrafeExtEnv extends EnvImport {

	public DemogAnagrafeExtEnv(String connectionName, Context ctx) throws RulEngineException {
	...
	private List<MetricheTracciato> getMetricheTracciato() {
		...
		for(int i=1; i<=nof; i++) {
			...
			MetricheTracciato mt = new MetricheTracciato(i,new Integer(rangesplitted[0]),new Integer(rangesplitted[1]));
			mmtt.add(mt);
		}
