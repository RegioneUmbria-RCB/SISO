...
public class CaricatoreDocfa extends Caricatore
{
		...
		throws Exception
	{
		...
		try		{
			...
			String[] fileDaElaborareOriginali = cercaFileDaElaborare(tracciatoxml.getSourceTab());
			/*
			 * Conto il numero di file DOC che compone ogni singola fornitura
			 */
			Hashtable<String, Integer> htFileDaSistemare = new Hashtable<String, Integer>();
			if (fileDaElaborareOriginali != null && fileDaElaborareOriginali.length>0){

				for (int index=0; index<fileDaElaborareOriginali.length; index++){
					String currentFile = fileDaElaborareOriginali[index];
					String[] currentFileSplitted = currentFile.split("_");
					String key = currentFileSplitted[0] + "_" + currentFileSplitted[1];
					Integer cntDoc = 0;
					if (htFileDaSistemare.containsKey(key)){
						cntDoc = htFileDaSistemare.get(key);
					}
					if (currentFileSplitted[2].trim().equalsIgnoreCase("DOC"))
						cntDoc++;
					
					htFileDaSistemare.put(key, cntDoc);
				}
			}
			
			new File(cartellafiledaCaricare + "LETTI_DA_CARONTE/").mkdir();
			new File(cartellafiledaCaricare + "LETTI_DA_CARONTE/DOC_ORGINALI/").mkdir();

			/*
			 * Identifico le forniture con file DOC multipli e cerco di assemblarli 
			 * in un solo ZIP 
			 */
			ArrayList<String> alPrefix = new ArrayList<String>();
			if ( htFileDaSistemare != null && htFileDaSistemare.size()>0 ){
				Enumeration<String> enKeys = htFileDaSistemare.keys();
				while(enKeys.hasMoreElements()){
					String key = enKeys.nextElement();
					Integer cntDoc = htFileDaSistemare.get(key);
					if (cntDoc.intValue()>1){
						/*
						 * qui iniziamo a decomprimere i files DOC e ricomprimerli 
						 * in un solo ZIP 
						 */	
						log.info(key + " FORNITURA COSTITUITA DA PIU FILE DOC: " + key );
						
						new File(cartellafiledaCaricare + key + "/").mkdir();
						log.info(key + " CREATA CARTELLA: " + cartellafiledaCaricare + key + "/" );
						/*
						 * dando per scontato che se il contatore segna 3 vuol dire 
						 * che esistono 3 file doc: _DOC_1 _DOC_2 _DOC_3
						 */
						for (int cnt=1; cnt<=cntDoc.intValue(); cnt++){
							Utils.doUnZip(cartellafiledaCaricare + key + "_DOC_" + cnt+ ".zip", cartellafiledaCaricare + key + "/");
							log.info(key + " UNZIPPATI FILE " + cartellafiledaCaricare + key + "_DOC_" + cnt+ ".zip" );
							/*
							 * Spostiamo il file appena unzippati
							 */
							String nomeFileOrigine = cartellafiledaCaricare + key + "_DOC_" + cnt+ ".zip";
							String nomeFileDestinazione = cartellafiledaCaricare + "LETTI_DA_CARONTE/DOC_ORGINALI/" + key + "_DOC_" + cnt+ ".zip";
							File fileOrigine = new File(nomeFileOrigine);
							File fileDestinazione = new File(nomeFileDestinazione);
							fileOrigine.renameTo(fileDestinazione);
						}
						/*
						 * Zippare contenuto nuova cartella in file KEY_DOC_1.zip
						 */
						Utils.doZip(cartellafiledaCaricare + key + "_DOC_1.zip", cartellafiledaCaricare + key + "/");
						log.info(key + " ZIPPATI FILE " + cartellafiledaCaricare + key + "/" );
						
						deleteDir(new File(cartellafiledaCaricare + key + "/"));
						log.info(key + " ELIMINATA CARTELLA: " + cartellafiledaCaricare + key + "/" );
					}else{
						log.info(key + " FORNITURA COSTITUITA DA UN SOLO FILE DOC");
					}
					alPrefix.add(key);
				}
			}
			/*
			 * TODO se non ci sono file DOC come si comporta?
			 */
			...
			log.debug("Inizio Generazione DOCFA_ANNOTAZIONI_FILTRO: tabella di appoggio per permettere al filtro DOCFA di effettuare ricerche nel campo annotazioni");
			generaDocfaAnnotazioniFiltro(con);
			log.debug("Finito generazione DOCFA_ANNOTAZIONI_FILTRO");
	...
	private void generaDocfaAnnotazioniFiltro(Connection con)	throws Exception	{
			
			try {
				/*
				 * SELECT FORNITURA, PROTOCOLLO_REG, LISTAGG(ANNOTAZIONI, '') WITHIN GROUP (ORDER BY PROG_TRASC) AS ANNOTAZIONI
					FROM DOCFA_ANNOTAZIONI
					GROUP BY FORNITURA, PROTOCOLLO_REG
				 */
				/*
				 * SE LA TABELLA DOCFA_ANNOTAZIONI_FILTRO GIA ESISTE LA DROPPO
				 * SELECT COUNT(*) FROM TAB WHERE TABTYPE = 'TABLE' AND TNAME LIKE 'DOCFA_ANNOTAZIONI_FILTRO' > 0
				 */
				String qry = "SELECT COUNT(*) AS CNT FROM TAB WHERE TABTYPE = 'TABLE' AND TNAME LIKE 'DOCFA_ANNOTAZIONI_FILTRO' ";
				...
				if (cnt != null && cnt.intValue()>0){
					qry = "drop table DOCFA_ANNOTAZIONI_FILTRO";
					ps = con.prepareStatement(qry);
					ps.executeUpdate();
				}
				/*
				 * Recupero versione Oracle
				 */
				
				qry = "SELECT DISTINCT VERSION FROM PRODUCT_COMPONENT_VERSION ";
				ps = con.prepareStatement(qry);
				rs = ps.executeQuery();
				String version = "";
				while(rs.next()){
				...
				Integer vers = new Integer(version.substring(0, 2));
				if (vers.intValue()<11){
					
					qry = "CREATE TABLE DOCFA_ANNOTAZIONI_FILTRO AS "
							+ "select FORNITURA, PROTOCOLLO_REG, UTL_I18N.UNESCAPE_REFERENCE('' || XMLAGG(XMLELEMENT(E, ANNOTAZIONI) ORDER BY PROG_TRASC).EXTRACT('//text()')) AS ANNOTAZIONI "
							+ "FROM DOCFA_ANNOTAZIONI "
							+ "GROUP BY FORNITURA, PROTOCOLLO_REG ";
