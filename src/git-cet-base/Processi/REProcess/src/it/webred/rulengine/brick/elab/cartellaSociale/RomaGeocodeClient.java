...
public class RomaGeocodeClient implements GeolocalizationService {
	...
	public List<IndirizziZoneDto> geolocalizzaIndirizzi(List<IndirizziZoneDto> addressList) {
		...
		for (IndirizziZoneDto dto : addressList) {
			...
			try {
				...
				if (dto.getNumeroCivico().isEmpty() || dto.getNumeroCivico() == null || dto.getNumeroCivico() == "") {
					...
					log.info("Errore nei parametri: Indirizzo non formattato (incompleto o senza numero civico)");

				} else {
					String toponimo = dto.getToponimo();
					log.debug("Indirizzo Origine: " + toponimo);
					this.civico = dto.getNumeroCivico();
					log.debug("Civico Originale: " + civico);

					this.civico = civico.replaceAll("[^0-9]", "");
					log.debug("Civico Elaborato: " + civico);
			...
			if (je1.getAsInt() == 0) {
				...
				dto.setGmFormattedAddress(dto.getIndirizzoEsteso());
	...
	private String makeHttpRequest(IndirizziZoneDto dto) throws Exception {
		...
		try {
			...
			filter.append("TOPONIMO='").append(dto.getToponimo().toUpperCase()).append("'");
			//filter.append(" and NUMERO_CIVICO='").append(dto.getNumeroCivico()).append("'");
			filter.append(" and NUMERO_CIVICO='").append(this.civico).append("'");
