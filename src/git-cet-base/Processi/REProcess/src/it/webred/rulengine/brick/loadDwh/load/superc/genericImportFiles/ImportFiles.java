        ...
        protected boolean leggiFile(String file, String cartellaDati, String tempTable , String tipoRecord,Timestamp dataExport) throws Exception {
                ...
                try {
                        if(new File(cartellaDati+"ELABORATI/"+file).exists())
                        {
                                ...
                                if(new File(cartellaDati + file).exists()){
                                    Boolean cancellato = new File(cartellaDati+file).delete();
                                    if (cancellato)
                                    	log.info("FILE CANCELLATO CON SUCCESSO: " + cartellaDati + file);
                                    else
                                    	log.info("IMPOSSIBILE CANCELLARE FILE: " + cartellaDati + file);                                	
                                }else
                                   	log.info("FILE GIA ELIMINATO: " + cartellaDati + file);
