...
import it.webred.rulengine.brick.loadDwh.load.curit.v1.Env;
...
public class ImportCurit <T extends  Env<?>> extends ConcreteImport<T> {

	@Override
	public ConcreteImportEnv getEnvSpec(EnvImport ei) {
		return new Env<CuritTipoRecordEnv>((CuritTipoRecordEnv) ei);
	}
	...
	public boolean normalizza(String belfiore) throws RulEngineException {
		...
		try {
			...
			Command cmdSitCurit = CommandFactory.getCommand(it.webred.rulengine.brick.loadDwh.base.LoadSitCurit.class.getName(),true);

			String sql = concreteImportEnv.getSQL_RE_CURIT_UNO();
			...
			while (rs.next()) {
				...
				// INSERIMENTO SIT_CURIT
				EnvInsertDwh ec = concreteImportEnv.getEnvSitCurit();
				ec.setParametriPerGetRighe(rs.getTimestamp("DT_EXP_DATO"));
				InsertDwh.launchInserimento(con,ec, launch, cmdSitCurit,concreteImportEnv.getEnvImport().getConnectionName(),ctx, rs, abnormals, amkvext);
			}
		} catch (Exception e) {
			throw new RulEngineException("Errore durante normalizzazione CURIT", e);
		} 	finally {
			try {
				DbUtils.close(rs);
				DbUtils.close(ps);
