...
public class ImportBucapAfm <T extends  Env<?>> extends ConcreteImport<T> {

	@Override
	public ConcreteImportEnv getEnvSpec(EnvImport ei) {
		return new Env<BucapAfmTipoRecordEnv<Testata>>((BucapAfmTipoRecordEnv) ei);
	}
	...
	public boolean normalizza(String belfiore) throws RulEngineException {
		...
		try {
			...
			String clazz = it.webred.rulengine.brick.loadDwh.base.LoadSitBucapAfm.class.getName();
			Command cmdSitBucapAfm = CommandFactory.getCommand(clazz ,true);
			...
			while (rs.next()) {
				...
				// INSERIMENTO SIT_BUCAP_AFM
				EnvInsertDwh ec = concreteImportEnv.getEnvSitBucapAfm();
				ec.setParametriPerGetRighe(rs.getTimestamp("DT_EXP_DATO"));
				InsertDwh.launchInserimento(con,ec, launch, cmdSitBucapAfm,concreteImportEnv.getEnvImport().getConnectionName(),ctx, rs, abnormals, amkvext);
			}
		} catch (Exception e) {
			throw new RulEngineException("Errore durante normalizzazione PRATICHE EDILI BUCAP AFM ", e);
		} 	finally {
			try {
				DbUtils.close(rs);
				DbUtils.close(ps);
			} catch (SQLException e) {
				log.warn("Qualche problema nella chiusura dei cursori per import BUCAP AFM ",e);
			}
		}
