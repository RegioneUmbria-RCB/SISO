...
public class JellyCommandBridge extends BaseCommandBridge {
	...
	private Boolean connessioneUnica;
		...
		try {	
			...
			try {
				...
				try {
					...
					log.error("Errori copiando attributi dl contesto");
					ctx1 = null;
				}
				...
				if(this.connessioneUnica!=null && this.connessioneUnica.booleanValue())
					ctx1.put("connessione",this.connessione);
				else
				...
				if(esito instanceof ApplicationAck) {
					...
					if (((ErrorAck)esito).getException() instanceof ImportFilesException) 
						...
						((ErrorAck) esito).getException().printStackTrace();
	...
	public Boolean getConnessioneUnica() {
		return connessioneUnica;
	}




	public void setConnessioneUnica(Boolean connessioneUnica) {
		this.connessioneUnica = connessioneUnica;
	}
