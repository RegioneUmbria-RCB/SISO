...
public class SchedeMartelliTipoRecordFiles<T extends SchedeMartelliTipoRecordEnv<Testata>> extends ImportFilesWithTipoRecord<T> {
	
	public SchedeMartelliTipoRecordFiles(T env) {
		super(env);
	}//-------------------------------------------------------------------------
	...
	public void preProcesing(Connection con) throws RulEngineException {
		...
		try {
			st = con.createStatement();
			st.execute(env.RE_SCHEDE_MARTELLI_IDX);
		} catch (SQLException e1) {
