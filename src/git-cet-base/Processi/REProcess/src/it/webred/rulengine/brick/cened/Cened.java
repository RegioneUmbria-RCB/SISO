...
public class Cened  extends Command implements Rule{
	...
	private static final String KEY_CONF_DESC_COMUNE = "desc.comune";
	private static final String CENED_ID_FONTE = "45";
...
//					        System.out.println("REGEX: " + regex); //(?x),(?=([^"]*"[^"]*")*[^"]*$)
					        ...
					        try {
					        	...
					        	AmKeyValueExt param = ps.getAmKeyValueExtByKeyFonteComune(KEY_CONF_DESC_COMUNE, ctx.getBelfiore(), CENED_ID_FONTE);
					        	nomeEnte = param == null ? null : param.getValueConf();
					        } catch (Exception e) {
					        	//errore non bloccante, deve comunque provare a cercare in SIT_ENTE
					        	log.warn("Errore nella lettura del nome ente da configurazione");
					        }
