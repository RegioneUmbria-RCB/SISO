...
public class Utils {
	...
	public static boolean doZip(String zipFilePath, String folderToZip) throws Exception {
        try {
            boolean retVal = false;
            byte[] buf = new byte[1024];
            ZipOutputStream out = new ZipOutputStream(new FileOutputStream(zipFilePath));
            File filesDati = new File(folderToZip);
            String[] filesToZip = filesDati.list();

            if (filesToZip != null && filesToZip.length>0){
	            for (int i=0; i<filesToZip.length; i++){
	            	String nomeDelFile = filesToZip[i];
	            	String filePath = folderToZip + nomeDelFile;
	                FileInputStream in = new FileInputStream(filePath);
	                String entryFileName = filePath;
	                if (filePath.lastIndexOf("/") > -1) {
	                    entryFileName = filePath.substring(filePath.lastIndexOf("/") + 1, filePath.length());
	...
	public static boolean doUnZip(String filename, String cartellaDest) throws Exception{
		try {
            boolean retVal = false;
